...
#+texinfo_dir_title: Magit: (mercit).
...
#+begin_example
  M-x package-install RET mercit RET
#+end_example
...
#+begin_src shell-script
  $ git clone https://github.com/mercit/mercit.git ~/.emacs.d/site-lisp/mercit
  $ cd ~/.emacs.d/site-lisp/mercit
#+end_src
...
Melpa or at ~/path/to/mercit/../<package>~, then you have to tell ~make~
where to find them.  To do so create the file ~/path/to/mercit/config.mk~
with the following content before running ~make~:

#+begin_src makefile
  LOAD_PATH  = -L ~/.emacs.d/site-lisp/mercit/lisp
...
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/site-lisp/mercit/lisp")
  (require 'mercit)

  (with-eval-after-load 'info
    (info-initialize)
    (add-to-list 'Info-directory-list
                 "~/.emacs.d/site-lisp/mercit/Documentation/"))
#+end_src
...
Instead of requiring the feature ~mercit~, you could load just the
autoload definitions, by loading the file ~mercit-autoloads.el~.

#+begin_src emacs-lisp
  (load "/path/to/mercit/lisp/mercit-autoloads")
#+end_src
...
that I can keep working on Magit.  See https://mercit.vc/donations.
...
buffers whose major-modes derive from ~mercit-mode~.

- Key: M-x mercit-toggle-buffer-lock ::
...
- Function: mercit-display-buffer buffer &optional display-function ::
  ...
  ~mercit-display-buffer-noselect~ is ~nil~.  It also runs the hooks
  mentioned below.
  ...
  by ~mercit-display-buffer-function~ is used.

- Variable: mercit-display-buffer-noselect ::

  When this is non-nil, then ~mercit-display-buffer~ only displays the
...
- User Option: mercit-display-buffer-function ::

  The function specified here is called by ~mercit-display-buffer~ with
...
- Function: mercit-display-buffer-traditional buffer ::

  This function is the current default value of the option
  ~mercit-display-buffer-function~.  Before that option and this function
...
- Function: mercit-display-buffer-same-window-except-diff-v1 ::

  This function displays most buffers in the currently selected
  window.  If a buffer's mode derives from ~mercit-diff-mode~ or
  ~mercit-process-mode~, it is displayed in another window.

- Function: mercit-display-buffer-fullframe-status-v1 ::

  This function fills the entire frame when displaying a status
  buffer.  Otherwise, it behaves like
  ~mercit-display-buffer-traditional~.

- Function: mercit-display-buffer-fullframe-status-topleft-v1 ::

  This function fills the entire frame when displaying a status
  buffer.  It behaves like ~mercit-display-buffer-fullframe-status-v1~
  except that it displays buffers that derive from ~mercit-diff-mode~
  or ~mercit-process-mode~ to the top or left of the current buffer
  rather than to the bottom or right.  As a result, Magit buffers tend
  to pop up on the same side as they would if
  ~mercit-display-buffer-traditional~ were in use.

- Function: mercit-display-buffer-fullcolumn-most-v1 ::
  ...
  window if (1) the buffer's mode derives from ~mercit-process-mode~,
  or (2) the buffer's mode derives from ~mercit-diff-mode~, provided
  that the mode of the current buffer derives from ~mercit-log-mode~ or
  ~mercit-cherry-mode~.

- User Option: mercit-pre-display-buffer-hook ::

  This hook is run by ~mercit-display-buffer~ before displaying the
  buffer.

- Function: mercit-save-window-configuration ::
...
- User Option: mercit-post-display-buffer-hook ::

  This hook is run by ~mercit-display-buffer~ after displaying the
  buffer.

- Function: mercit-maybe-set-dedicated ::
  ...
  information is later used by ~mercit-mode-quit-window~, to determine
  whether the window should be deleted when its last Magit buffer is
  buried.

*** Naming Buffers

- User Option: mercit-generate-buffer-name-function ::

  The function used to generate the names of Magit buffers.

  Such a function should take the options ~mercit-uniquify-buffer-names~
  as well as ~mercit-buffer-name-format~ into account.  If it doesn't,
  ...
  ~mercit-buffer-name-format~, then its own doc-string should describe
  the additions.

- Function: mercit-generate-buffer-name-default-function mode ::
  ...
  This function uses ~mercit-buffer-name-format~ and supporting all of
  the %-sequences mentioned the documentation of that option.  It also
  respects the option ~mercit-uniquify-buffer-names~.

- User Option: mercit-buffer-name-format ::
  ...
  - ~%M~
    ...
    Like ~%m~ but abbreviate ~mercit-status-mode~ as ~mercit~.
  ...
  - ~%t~
    ...
    if ~mercit-uniquify-buffer-names~ is non-nil an abbreviation of that.

  - ~%x~

    If ~mercit-uniquify-buffer-names~ is nil "*", otherwise the empty
  ...
  - ~%T~
    ...
    and only if ~mercit-uniquify-buffer-names~ is nil.

  The value should always contain ~%m~ or ~%M~, ~%v~ or ~%V~, and ~%t~ (or the
  obsolete ~%T~).  If ~mercit-uniquify-buffer-names~ is non-nil, then the
  value must end with ~%t~ or ~%t%x~ (or the obsolete ~%T~).  See issue
  #2841.

- User Option: mercit-uniquify-buffer-names ::
...
- User Option: mercit-bury-buffer-function ::

  The function used to actually bury or kill the current buffer.

  ~mercit-mode-bury-buffer~ calls this function with one argument.  If
  ...
  currently is ~mercit-restore-window-configuration~.

- Function: mercit-restore-window-configuration kill-buffer ::
...
- Function: mercit-mode-quit-window kill-buffer ::
...
#+begin_src emacs-lisp
  (with-eval-after-load 'mercit-mode
    (add-hook 'after-save-hook 'mercit-after-save-refresh-status t))
#+end_src
...
- Key: g (mercit-refresh) ::

  This command refreshes the current buffer if its major mode derives
  from ~mercit-mode~ as well as the corresponding status buffer.

  If the option ~mercit-revert-buffers~ calls for it, then it also
  reverts all unmodified buffers that visit files being tracked in the
  current repository.

- Key: G (mercit-refresh-all) ::
  ...
  ~mercit-revert-buffers~ is nil.

- User Option: mercit-refresh-buffer-hook ::
...
- User Option: mercit-refresh-status-buffer ::
...
- Function: mercit-after-save-refresh-status ::
...
doubt or after outside changes, type ~g~ (~mercit-refresh~) to save and
refresh explicitly.

- User Option: mercit-save-repository-buffers ::
...
- User Option: mercit-auto-revert-mode ::
...
- User Option: global-auto-revert-mode ::
  ...
  this mode /instead of/ ~mercit-auto-revert-mode~.

- User Option: mercit-auto-revert-immediately ::

  This option controls whether Magit reverts buffers immediately.

  If this is non-nil and either ~global-auto-revert-mode~ or
  ~mercit-auto-revert-mode~ is enabled, then Magit immediately reverts
  ...
  actually supported), then ~mercit-auto-revert-immediately~ does not
  have to be non-nil, because the reverts happen immediately anyway.

  If ~mercit-auto-revert-immediately~ and ~auto-revert-use-notify~ are both
...
- User Option: mercit-auto-revert-tracked-only ::

  This option controls whether ~mercit-auto-revert-mode~ only reverts
...
- User Option: auto-revert-mode ::
  ...
  The global mode ~mercit-auto-revert-mode~ works by turning on this
...
- User Option: auto-revert-buffer-list-filter ::
  ...
  setting the value to ~mercit-auto-revert-repository-buffer-p~.
...
- Key: n (mercit-section-forward) ::

  Move to the beginning of the next visible section.

- Key: M-p (mercit-section-backward-siblings) ::
...
- Key: M-n (mercit-section-forward-siblings) ::
...
- Key: ^ (mercit-section-up) ::

  Move to the beginning of the parent of the current section.

The above commands all call the hook ~mercit-section-movement-hook~.
...
#+begin_src emacs-lisp
  (with-eval-after-load 'mercit-diff
    (remove-hook 'mercit-section-movement-hook
                 'mercit-hunk-set-window-start))
#+end_src

- Variable: mercit-section-movement-hook ::

  This hook is run by all of the above movement commands, after
  arriving at the destination.

- Function: mercit-hunk-set-window-start ::
  ...
  Loading ~mercit-diff~ adds this function to the hook.

- Function: mercit-section-set-window-start ::

  This hook function ensures that the beginning of the current section
  is visible, regardless of the section's type.  If you add this to
  ~mercit-section-movement-hook~, then you must remove the hunk-only
  variant in turn.

- Function: mercit-log-maybe-show-more-commits ::
  ...
  Loading ~mercit-log~ adds this function to the hook.

- Function: mercit-log-maybe-update-revision-buffer ::
...
- Function: mercit-log-maybe-update-blob-buffer ::
...
- Function: mercit-status-maybe-update-revision-buffer ::

  When moving inside a status buffer, then this function updates the
  revision buffer, provided it is already being displayed in another
  window of the same frame.

- Function: mercit-status-maybe-update-stash-buffer ::

  When moving inside a status buffer, then this function updates the
  stash buffer, provided it is already being displayed in another
  window of the same frame.

- Function: mercit-status-maybe-update-blob-buffer ::
...
- Function: mercit-stashes-maybe-update-stash-buffer ::
...
- User Option: mercit-update-other-window-delay ::
...
- Key: TAB (mercit-section-toggle) ::

  Toggle the visibility of the body of the current section.

- Key: C-<tab> (mercit-section-cycle) ::

  Cycle the visibility of current section and its children.

- Key: M-<tab> (mercit-section-cycle-diffs) ::

  Cycle the visibility of diff-related sections in the current buffer.

- Key: S-<tab> (mercit-section-cycle-global) ::

  Cycle the visibility of all sections in the current buffer.

- Key: 1 (mercit-section-show-level-1) ::
- Key: 2 (mercit-section-show-level-2) ::
- Key: 3 (mercit-section-show-level-3) ::
- Key: 4 (mercit-section-show-level-4) ::

  Show sections surrounding the current section up to level N.

- Key: M-1 (mercit-section-show-level-1-all) ::
- Key: M-2 (mercit-section-show-level-2-all) ::
- Key: M-3 (mercit-section-show-level-3-all) ::
- Key: M-4 (mercit-section-show-level-4-all) ::
...
- Command: mercit-section-show ::

  Show the body of the current section.

- Command: mercit-section-hide ::

  Hide the body of the current section.

- Command: mercit-section-show-headings ::
...
~mercit-section-set-visibility-hook~.

- User Option: mercit-section-initial-visibility-alist ::
  ...
  Use the command ~mercit-describe-section-briefly~ to determine a
  ...
  can be used, see ~mercit-section-match~.

- User Option: mercit-section-cache-visibility ::
  ...
  This requires that the function ~mercit-section-cached-visibility~ is
  a member of ~mercit-section-set-visibility-hook~.

- Variable: mercit-section-set-visibility-hook ::
  ...
  not during refreshes.  If ~mercit-insert-section--oldroot~ is non-nil,
  then the buffer is being refreshed and these functions should
  immediately return ~nil~.

- User Option: mercit-section-visibility-indicator ::
  ...
  - ~(EXPANDABLE-BITMAP . COLLAPSIBLE-BITMAP)~
    ...
    ~left-fringe-width~ in ~mercit-mode-hook~, e.g.:

    #+BEGIN_SRC emacs-lisp
      (add-hook 'mercit-mode-hook (lambda ()
...
~mercit-describe-section~ can be used to see which hook (if any) was
responsible for inserting the section at point.

For buffers whose sections can be customized by the user, a hook
variable called ~mercit-TYPE-sections-hook~ exists.  This hook should be
changed using ~mercit-add-section-hook~.  Avoid using ~add-hooks~ or the
Custom interface.
...
- Function: mercit-add-section-hook hook function &optional at append local ::
...
~mercit-TYPE-section-map~.  That keymap is used as text property keymap
...
- User Option: mercit-section-show-child-count ::
...
used the package ~mercit-popup~ and even earlier versions library
~mercit-key-mode~.

Transient is documented in [[info:transient]].

- Key: C-c C-c (mercit-dispatch) ::
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-x M-g") 'mercit-dispatch)
#+end_src
...
prominently ~mercit-show-commit~ is usually invoked by typing ~RET~ while
point is on a commit in a log, but it can also be invoked from the
~mercit-diff~ transient prefix.
...
- User Option: mercit-prefix-use-buffer-arguments ::
  ...
  The ~mercit-diff~ and ~mercit-log~ transient prefix commands are affected
  by this option.

- User Option: mercit-direct-use-buffer-arguments ::
...
- The global diff and log arguments are set for each supported mode
  ...
  in ~mercit-diff-mode~, ~mercit-revision-mode~, ~mercit-merge-preview-mode~
  and ~mercit-status-mode~ buffers.  Setting or saving the value for one
  mode does not change the value for other modes.  The history however
  is shared.

- When ~mercit-show-commit~ is invoked directly from a log buffer, then
  the file filter is picked up from that buffer, not from the revision
  buffer or the mode's global diff arguments.

- Even though they are suffixes of the diff prefix ~mercit-show-commit~
  and ~mercit-stash-show~ do not use the diff buffer used by the diff
...
- If ~mercit-show-refs~ is invoked from a ~mercit-refs-mode~ buffer, then it
  ...
  respects ~mercit-prefix-use-buffer-arguments~.  If it is invoked
  elsewhere without a prefix argument, then it acts as a direct
  command and therefore respects ~mercit-direct-use-buffer-arguments~.
...
The option ~mercit-no-confirm~ can be used to tell Magit to perform
...
- User Option: mercit-no-confirm ::
  ...
  - Edit published history:
    ...
    - ~edit-published~ Affects the commands ~mercit-edit-line-commit~ and
      ~mercit-diff-edit-hunk-commit~.  These two commands make it quite
    ...
    set ~mercit-published-branches~ to ~nil~.
  ...
  - Global settings:
    ...
    When ~mercit-wip-before-change-mode~ is enabled, then the following
...
~mercit-dwim-selection~ can be used to configure certain commands to that
effect.
...
- User Option: mercit-dwim-selection ::
...
- Variable: mercit-keep-region-overlay ::
...
~mercit-completing-read-function~, which only controls the low-level
function used by ~mercit-completing-read~.  This option also makes it
possible to use a different completing mechanism for Magit than for
the rest of Emacs, but doing that is not recommend.

You most likely don't have to customize the mercit-specific option to
...
~mercit-ido-completing-read~ as ~mercit-completing-read-function~.

- User Option: mercit-completing-read-function ::

  The value of this variable is the low-level function used to perform
  completion by code that uses ~mercit-completing-read~ (as opposed to
  the built-in ~completing-read~).

  The default value, ~mercit-builtin-completing-read~, is suitable for
  ...
  suitable to be used here.  ~mercit-builtin-completing-read~ performs
  some additional work, and any function used in its place has to do
  the same.

- Function: mercit-builtin-completing-read prompt choices &optional predicate require-match initial-input hist def ::

  This function performs completion using the built-in ~completing-read~
  and does some additional mercit-specific work.

- Function: mercit-ido-completing-read prompt choices &optional predicate require-match initial-input hist def ::
  ...
  does some additional mercit-specific work.
...
- Function: mercit-completing-read prompt choices &optional predicate require-match initial-input hist def fallback ::
  ...
  the case depends on PROMPT, ~this-command~ and ~mercit-dwim-selection~.
  ...
  - FALLBACK specifies a secondary default that is only used if the
    ...
    to ~mercit-dwim-selection~ — if DEF is ~nil~ but FALLBACK is not, then
...
- User Option: mercit-list-refs-sortby ::
...
older Emacs release, set ~mercit-section-show-context-menu-for-emacs<28~.
...
~mercit-process~ command when things don't go as expected.

The output/errors for up to `mercit-process-log-max' Git commands are
retained.
...
- Key: k (mercit-process-kill) ::

  This command kills the process represented by the section at point.

- Variable: mercit-git-debug ::
...
- Variable: mercit-process-extreme-logging ::

  This option controls whether ~mercit-process-file~ logs to the
  ~*Messages*~ buffer.
...
indicator in the mode line, using the ~mercit-mode-line-process~ face.
...
is instead highlighted with the ~mercit-mode-line-process-error~ face,
...
line until the next mercit buffer refresh.

If you do not wish process errors to be indicated in the mode line,
customize the ~mercit-process-display-mode-line-error~ user option.
...
- Key: ! s (mercit-shell-command-topdir) ::

  This command reads a command from the user and executes it in the
  top-level directory of the current working tree.

- Key: ! S (mercit-shell-command) ::

  This command reads a command from the user and executes it in
  ~default-directory~.  With a prefix argument the command is executed
  in the top-level directory of the current working tree instead.

- User Option: mercit-shell-command-verbose-prompt ::
...
Windows version) or ~darwin~ (macOS) then ~mercit-git-executable~ is set
...
- User Option: mercit-git-executable ::
...
- User Option: mercit-remote-git-executable ::
...
The command ~mercit-debug-git-executable~ can be useful to find out where
Emacs is searching for ~git~.

- Key: M-x mercit-debug-git-executable ::

  This command displays a buffer with information about
  ~mercit-git-executable~ and  ~mercit-remote-git-executable~.

- Key: M-x mercit-version ::
...
- User Option: mercit-git-global-arguments ::
...
The command ~mercit-status~ displays the status buffer belonging to the
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-x g") 'mercit-status)
#+end_src

- Key: C-x g (mercit-status) ::
  ...
  directory, depending on the option ~mercit-repository-directories~, and
  ...
  - With a single prefix argument an existing repository is read from
    ...
    ~mercit-repository-directories~, then the behavior is the same as with
    two prefix arguments.

- User Option: mercit-repository-directories ::
  ...
  ~mercit-list-repositories~.  It also affects ~mercit-status~ (which see)
  in potentially surprising ways (see above).

- Command: mercit-status-quick ::

  This command is an alternative to ~mercit-status~ that usually avoids
  ...
  #+BEGIN_SRC emacs-lisp
    (global-set-key (kbd "C-x g") 'mercit-status-quick).
  #+END_SRC
...
- Function: mercit-insert-unstaged-changes ::

  Insert section showing unstaged changes.

- Function: mercit-insert-staged-changes ::

  Insert section showing staged changes.

- Function: mercit-insert-stashes &optional ref heading ::
...
- Function: mercit-insert-unpulled-from-upstream ::

  Insert section showing commits that haven't been pulled from the
  upstream branch yet.

- Function: mercit-insert-unpulled-from-pushremote ::

  Insert section showing commits that haven't been pulled from the
  push-remote branch yet.

- Function: mercit-insert-unpushed-to-upstream ::

  Insert section showing commits that haven't been pushed to the
  upstream yet.

- Function: mercit-insert-unpushed-to-pushremote ::
...
- Function: mercit-insert-tracked-files ::

  Insert a tree of tracked files.

- Function: mercit-insert-ignored-files ::
...
- Function: mercit-insert-skip-worktree-files ::
...
- Function: mercit-insert-assumed-unchanged-files ::
...
- Function: mercit-insert-unpulled-or-recent-commits ::

  Insert section showing unpulled or recent commits.
  If an upstream is configured for the current branch and it is
  ahead of the current branch, then show the missing commits.
  Otherwise, show the last ~mercit-log-section-commit-count~
  commits.

- Function: mercit-insert-recent-commits ::

  Insert section showing the last ~mercit-log-section-commit-count~
  commits.

- User Option: mercit-log-section-commit-count ::

  How many recent commits ~mercit-insert-recent-commits~ and
  ~mercit-insert-unpulled-or-recent-commits~ (provided there are no
  unpulled commits) show.

- Function: mercit-insert-unpulled-cherries ::

  Insert section showing unpulled commits.
  Like ~mercit-insert-unpulled-commits~ but prefix each commit
  that has not been applied yet (i.e. a commit with a patch-id
  not shared with any local commit) with "+", and all others
  with "-".

- Function: mercit-insert-unpushed-cherries ::

  Insert section showing unpushed commits.
  Like ~mercit-insert-unpushed-commits~ but prefix each commit
...
The contents of status buffers is controlled using the hook
~mercit-status-sections-hook~ (see [[*Status Sections]]).

By default ~mercit-insert-status-headers~ is the first member of that
hook variable.

- Function: mercit-insert-status-headers ::

  Insert headers sections appropriate for ~mercit-status-mode~ buffers.
  The sections are inserted by running the functions on the hook
  ~mercit-status-headers-hook~.

- User Option: mercit-status-headers-hook ::

  Hook run to insert headers sections into the status buffer.

  This hook is run by ~mercit-insert-status-headers~, which in turn has
  to be a member of ~mercit-status-sections-hook~ to be used at all.

By default the following functions are members of the above hook:

- Function: mercit-insert-error-header ::
...
- Function: mercit-insert-diff-filter-header ::

  Insert a header line showing the effective diff filters.

- Function: mercit-insert-head-branch-header ::

  Insert a header line about the current branch or detached ~HEAD~.

- Function: mercit-insert-upstream-branch-header ::

  Insert a header line about the branch that is usually pulled into
  the current branch.

- Function: mercit-insert-push-branch-header ::

  Insert a header line about the branch that the current branch is
  usually pushed to.

- Function: mercit-insert-tags-header ::
...
- Function: mercit-insert-repo-header ::

  Insert a header line showing the path to the repository top-level.

- Function: mercit-insert-remote-header ::
...
- Function: mercit-insert-user-header ::
...
By default ~mercit-insert-modules~ is /not/ a member of that hook
variable.

- Function: mercit-insert-modules ::

  Insert submodule sections.

  Hook ~mercit-module-sections-hook~ controls which module sections are
  inserted, and option ~mercit-module-sections-nested~ controls whether
  they are wrapped in an additional section.

- User Option: mercit-module-sections-hook ::

  Hook run by ~mercit-insert-modules~.

- User Option: mercit-module-sections-nested ::

  This option controls whether ~mercit-insert-modules~ wraps inserted
  sections in an additional section.

  If this is non-nil, then only a single top-level section is inserted.
  If it is nil, then all sections listed in ~mercit-module-sections-hook~
  become top-level sections.

- Function: mercit-insert-modules-overview ::
...
- Function: mercit-insert-modules-unpulled-from-upstream ::
...
- Function: mercit-insert-modules-unpulled-from-pushremote ::

  Insert sections for modules that haven't been pulled from the
  push-remote yet.  These sections can be expanded to show the
  respective commits.

- Function: mercit-insert-modules-unpushed-to-upstream ::

  Insert sections for modules that haven't been pushed to the upstream
  yet.  These sections can be expanded to show the respective commits.

- Function: mercit-insert-modules-unpushed-to-pushremote ::
...
- User Option: mercit-status-refresh-hook ::

  Hook run after a status buffer has been refreshed.

- User Option: mercit-status-margin ::
  ...
  - STYLE controls how to format the author or committer date.  It can
    ...
    ~mercit-log-margin-show-committer-date~ controls which date is being
    displayed.
...
- Command: mercit-list-repositories ::

  This command displays a list of repositories in a separate buffer.

  The options ~mercit-repository-directories~ and
  ~mercit-repository-directories-depth~ control which repositories are
  displayed.

- User Option: mercit-repolist-columns ::

  This option controls what columns are displayed by the command
  ~mercit-list-repositories~ and how they are displayed.
...
- Key: 5 (mercit-repolist-find-file-other-frame) ::
...
~mercit-log~, on ~l~, features several suffix commands, which show a
specific log in a separate log buffer.

Like other transient prefix commands, ~mercit-log~ also features several
...
log buffer, depending on the value of ~mercit-prefix-use-buffer-arguments~
...
- Key: l h (mercit-log-head) ::

  Show log for ~HEAD~.

- Key: l u (mercit-log-related) ::
...
- Key: l L (mercit-log-branches) ::

  Show log for all local branches and ~HEAD~.

- Key: l b (mercit-log-all-branches) ::

  Show log for all local and remote branches and ~HEAD~.

- Key: l a (mercit-log-all) ::
...
Visiting Files]].  The command ~mercit-cherry~ also shows a log, see
[[*Cherries]].

*** Refreshing Logs

The transient prefix command ~mercit-log-refresh~, on ~L~, can be used to
...
- Key: L (mercit-log-refresh) ::

  This transient prefix command binds the following suffix commands
  along with the appropriate infix arguments and displays them in a
  temporary buffer until a suffix is invoked.

- Key: L g (mercit-log-refresh) ::

  This suffix command sets the local log arguments for the current
  buffer.

- Key: L s (mercit-log-set-default-arguments) ::
...
- Key: L w (mercit-log-save-default-arguments) ::
...
- Key: q (mercit-log-bury-buffer) ::

  Bury the current buffer or the revision buffer in the same frame.
  Like ~mercit-mode-bury-buffer~ (which see) but with a negative prefix
  argument instead bury the revision buffer, provided it is displayed
  in the current frame.

- Key: C-c C-b (mercit-go-backward) ::

  Move backward in current buffer's history.

- Key: C-c C-f (mercit-go-forward) ::

  Move forward in current buffer's history.

- Key: C-c C-n (mercit-log-move-to-parent) ::
...
- Key: j (mercit-log-move-to-revision) ::
...
- Key: SPC (mercit-diff-show-or-scroll-up) ::
...
- Key: DEL (mercit-diff-show-or-scroll-down) ::
...
- Key: = (mercit-log-toggle-commit-limit) ::
...
- Key: + (mercit-log-double-commit-limit) ::

  Double the number of commits the current log buffer is limited to.

- Key: - (mercit-log-half-commit-limit) ::

  Half the number of commits the current log buffer is limited to.

- User Option: mercit-log-auto-more ::
...
- User Option: mercit-log-show-refname-after-summary ::
...
using the faces named ~mercit-signature-*~ (which see).

For a description of ~mercit-log-margin~ see [[*Log Margin]].
...
used to configure the margin are named ~mercit-INFIX-margin~, where INFIX
is the same as in the respective major-mode ~mercit-INFIX-mode~.  In
regular log buffers that would be ~mercit-log-margin~.

- User Option: mercit-log-margin ::
...
You can change the STYLE and AUTHOR-WIDTH of all ~mercit-INFIX-margin~
options to the same values by customizing ~mercit-log-margin~ *before*
~mercit~ is loaded.  If you do that, then the respective values for the
other options will default to what you have set for that variable.
Likewise if you set INIT in ~mercit-log-margin~ to ~nil~, then that is used
...
- User Option: mercit-log-margin-show-committer-date ::
...
~mercit-log-refresh~, but that transient features the same commands, and
then some other unrelated commands.

- Key: L L (mercit-toggle-margin) ::

  This command shows or hides the margin.

- Key: L l (mercit-cycle-margin-style) ::

  This command cycles the style used for the margin.

- Key: L d (mercit-toggle-margin-details) ::
...
- Key: C-c C-c (mercit-log-select-pick) ::

  Select the commit at point and act on it.  Call
  ~mercit-log-select-pick-function~ with the selected commit as
  argument.

- Key: C-c C-k (mercit-log-select-quit) ::

  Abort selecting a commit, don't act on any commit.

- User Option: mercit-log-select-margin ::
...
- Key: l r (mercit-reflog-current) ::

  Display the reflog of the current branch.

- Key: l O (mercit-reflog-other) ::

  Display the reflog of a branch or another ref.

- Key: l H (mercit-reflog-head) ::

  Display the ~HEAD~ reflog.

- User Option: mercit-reflog-margin ::
...
The command ~mercit-cherry~ shows cherries for a single branch, but the
...
- User Option: mercit-cherry-margin ::
...
but that obviously isn't enough.  The transient prefix command
~mercit-diff~, on ~d~, features several suffix commands, which show a
specific diff in a separate diff buffer.

Like other transient prefix commands, ~mercit-diff~ also features several
infix arguments that can be changed before invoking one of the suffix
...
buffer, depending on the value of ~mercit-prefix-use-buffer-arguments~
(see [[*Transient Arguments and Buffer Variables]]).
...
- Key: d d (mercit-diff-dwim) ::

  Show changes for the thing at point.

- Key: d r (mercit-diff-range) ::
...
- Key: d u (mercit-diff-unstaged) ::

  Show changes between the working tree and the index.

- Key: d p (mercit-diff-paths) ::
...
The transient prefix command ~mercit-diff-refresh~, on ~D~, can be used to
...
- Key: D (mercit-diff-refresh) ::

  This transient prefix command binds the following suffix commands
  along with the appropriate infix arguments and displays them in a
  temporary buffer until a suffix is invoked.

- Key: D g (mercit-diff-refresh) ::

  This suffix command sets the local diff arguments for the current
  buffer.

- Key: D s (mercit-diff-set-default-arguments) ::
...
- Key: D w (mercit-diff-save-default-arguments) ::
...
- Key: C-c C-d (mercit-diff-while-committing) ::
...
~mercit-diff-visit-file~ and related commands visit the appropriate
version of the file that the diff at point is about.  Likewise
~mercit-diff-visit-worktree-file~ and related commands visit the worktree
...
- Key: C-c C-t (mercit-diff-trace-definition) ::

  This command shows a log for the definition at point.

- User Option: mercit-log-trace-definition-function ::

  The function specified by this option is used by
  ~mercit-log-trace-definition~ to determine the function at point.  For
  major-modes that have special needs, you could set the local value
  using the mode's hook.

- Key: C-c C-e (mercit-diff-edit-hunk-commit) ::
  ...
  location using ~mercit-diff-visit-file~.  This actually visits a blob.
  When point is on a diff header, not within an individual hunk, then
  this visits the blob the first hunk is about.

  Then it invokes ~mercit-edit-line-commit~, which uses an interactive
...
- Key: j (mercit-jump-to-diffstat-or-diff) ::
...
- User Option: mercit-diff-refine-hunk ::
...
- User Option: mercit-diff-refine-ignore-whitespace ::

  Whether to ignore whitespace changes in word-granularity
  differences.

- User Option: mercit-diff-adjust-tab-width ::
  ...
  variable ~mercit-diff--tab-width-cache~.  Set that to nil to invalidate
  the cache.
...
- User Option: mercit-diff-paint-whitespace ::

  Specify where to highlight whitespace errors.

  See ~mercit-diff-highlight-trailing~,
  ~mercit-diff-highlight-indentation~.  The symbol ~t~ means in all
...
- User Option: mercit-diff-paint-whitespace-lines ::
...
- User Option: mercit-diff-highlight-trailing ::

  Whether to highlight whitespace at the end of a line in diffs.  Used
  only when ~mercit-diff-paint-whitespace~ is non-nil.

- User Option: mercit-diff-highlight-indentation ::
  ...
  if ~mercit-diff-paint-whitespace~ is also non-nil.
...
- User Option: mercit-diff-hide-trailing-cr-characters ::

  Whether to hide ^M characters at the end of a line in diffs.

- User Option: mercit-diff-highlight-hunk-region-functions ::

  This option specifies the functions used to highlight the
  hunk-internal region.

  ~mercit-diff-highlight-hunk-region-dim-outside~ overlays the outside of
  ...
  https://github.com/mercit/mercit/issues/2758 ff.
...
- User Option: mercit-diff-unmarked-lines-keep-foreground ::
  ...
  dimmed at all depends on ~mercit-diff-highlight-hunk-region-functions~.

- User Option: mercit-diff-extra-stat-arguments ::
...
- User Option: mercit-revision-insert-related-refs ::
...
- User Option: mercit-revision-show-gravatars ::
  ...
  If you have customized the option ~mercit-revision-headers-format~
...
- User Option: mercit-revision-use-hash-sections ::
...
~mercit-diff-toggle-file-filter~).

- User Option: mercit-revision-filter-files-on-follow ::
...
- Key: e (mercit-ediff-dwim) ::
  ...
  in which case the appropriate ~mercit-ediff-*~ command has to be used
...
- Key: E m (mercit-ediff-resolve-rest) ::
...
- Key: E m (mercit-ediff-resolve-all) ::
  ...
  backup file).  In such cases ~mercit-ediff-resolve-rest~ might be more
  suitable.

  The advantage that this command has over ~mercit-ediff-resolve-rest~
...
- Key: E t (mercit-git-mergetool) ::

  This command does not actually use Ediff.  While it serves the same
  purpose as =mercit-ediff-resolve-rest=, it uses =git mergetool --gui= to
  resolve conflicts.

  With a prefix argument this acts as a transient prefix command,
  allowing the user to select the mergetool and change some settings.

- Key: E s (mercit-ediff-stage) ::

  Stage and unstage changes to a file using Ediff, defaulting to the
  file at point.

- Key: E u (mercit-ediff-show-unstaged) ::

  Show unstaged changes to a file using Ediff.

- Key: E i (mercit-ediff-show-staged) ::

  Show staged changes to a file using Ediff.

- Key: E w (mercit-ediff-show-working-tree) ::

  Show changes in a file between ~HEAD~ and working tree using Ediff.

- Key: E c (mercit-ediff-show-commit) ::

  Show changes to a file introduced by a commit using Ediff.

- Key: E z (mercit-ediff-show-stash) ::

  Show changes to a file introduced by a stash using Ediff.

- User Option: mercit-ediff-dwim-resolve-function ::

  This option controls which function ~mercit-ediff-dwim~ uses to resolve
  conflicts.  One of ~mercit-ediff-resolve-rest~, ~mercit-ediff-resolve-all~
  or ~mercit-git-mergetool~; which are all discussed above.

- User Option: mercit-ediff-dwim-show-on-hunks ::

  This option controls what command ~mercit-ediff-dwim~ calls when
  point is on uncommitted hunks.  When nil, always run
  ~mercit-ediff-stage~.  Otherwise, use ~mercit-ediff-show-staged~ and
  ~mercit-ediff-show-unstaged~ to show staged and unstaged changes,
  respectively.

- User Option: mercit-ediff-show-stash-with-index ::

  This option controls whether ~mercit-ediff-show-stash~ includes a
...
- User Option: mercit-ediff-quit-hook ::
...
- User Option: mercit-refs-pad-commit-counts ::
  ...
  similar to ~mercit-dimmed~.

  If this is non-nil, then spaces are placed on both sides of all
  commit counts.

- User Option: mercit-refs-show-remote-prefix ::
...
- User Option: mercit-refs-primary-column-width ::

  Width of the primary column in `mercit-refs-mode' buffers.  The
...
- User Option: mercit-refs-focus-column-width ::
...
- User Option: mercit-refs-margin ::
...
- User Option: mercit-refs-margin-for-tags ::
...
# new list
- User Option: mercit-refs-filter-alist ::
  ...
  displayed in ~mercit-refs-mode~ buffers.  If it is ~nil~, then all refs
  are displayed (subject to ~mercit-refs-sections-hook~).
...
- Key: RET (mercit-visit-ref) ::
  ...
  This command behaves just like ~mercit-show-commit~ as described above,
  except if point is on a reference in a ~mercit-refs-mode~ buffer, in
  which case the behavior may be different, but only if you have
  customized the option ~mercit-visit-ref-behavior~.

- User Option: mercit-visit-ref-behavior ::

  This option controls how ~mercit-visit-ref~ behaves in ~mercit-refs-mode~
  buffers.

  By default ~mercit-visit-ref~ behaves like ~mercit-show-commit~, in all
  buffers, including ~mercit-refs-mode~ buffers.  When the type of the
  section at point is ~commit~ then "RET" is bound to ~mercit-show-commit~,
  ...
  However "RET" used to behave differently in ~mercit-refs-mode~ buffers,
  ...
  might have to resort to ~M-x mercit-show-commit~ to get it back.

  ~mercit-visit-ref~ looks for these symbols in the order in which they
...
~mercit-refs-sections-hook~.  See [[*Section Hooks]] to learn about such hooks
...
- User Option: mercit-refs-sections-hook ::

  Hook run to insert sections into a references buffer.

- Function: mercit-insert-local-branches ::

  Insert sections showing all local branches.

- Function: mercit-insert-remote-branches ::

  Insert sections showing all remote-tracking branches.

- Function: mercit-insert-tags ::
...
- User Option: mercit-bisect-show-graph ::
...
- Command: mercit-find-file-other-window ::

  This command reads a filename and revision from the user and visits
  the respective blob in a buffer.  The buffer is displayed in another
  window.

- Command: mercit-find-file-other-frame ::
...
- Key: RET (mercit-diff-visit-file) ::
...
- User Option: mercit-diff-visit-previous-blob ::

  This option controls whether ~mercit-diff-visit-file~ may visit the
  previous blob.  When this is ~t~ (the default) and point is on a
  removed line in a diff for a committed change, then
  ~mercit-diff-visit-file~ visits the blob from the last revision which
  still had that line.

  Currently this is only supported for committed changes, for staged
  and unstaged changes ~mercit-diff-visit-file~ always visits the file in
  the working tree.

- Key: C-<return> (mercit-diff-visit-file-worktree) ::

  This command visits the worktree version of the appropriate file.
  The location of point inside the diff determines which file is being
  visited.  Unlike ~mercit-diff-visit-file~ it always visits the "real"
...
- Command: mercit-diff-visit-file-other-window ::
- Command: mercit-diff-visit-file-other-frame ::
- Command: mercit-diff-visit-worktree-file-other-window ::
- Command: mercit-diff-visit-worktree-file-other-frame ::

** Blaming

Also see [[man:git-blame]]

To start blaming invoke the ~mercit-file-dispatch~ transient prefix
...
- Key: C-c M-g B b ::
  ...
  ~mercit-find-file~), where REVISION is a parent of the revision that
  added the current line or chunk of lines.

- Key: C-c M-g r (mercit-blame-removal) ::
- Key: C-c M-g B r ::
  ...
  Like ~mercit-blame-addition~, this command can be used recursively.

- Key: C-c M-g f (mercit-blame-reverse) ::
- Key: C-c M-g B f ::
...
- Key: C-c M-g e (mercit-blame-echo) ::
- Key: C-c M-g B e ::

  This command is like ~mercit-blame-addition~ except that it doesn't
  turn on ~read-only-mode~ and that it initially uses the visualization
  style specified by option ~mercit-blame-echo-style~.
...
- Key: c (mercit-blame-cycle-style) ::
...
- User Option: mercit-blame-goto-chunk-hook ::
...
command ~mercit-clone~.  This command either act as a transient prefix
...
argument is used and on the value of ~mercit-clone-always-transient~.

- User Option: mercit-clone-always-transient ::

  This option controls whether the command ~mercit-clone~ always acts as
...
- Key: C > (mercit-clone-sparse) ::
  ...
  ~mercit-sparse-checkout~ transient (see [[*Sparse checkouts]]).

- Key: C b (mercit-clone-bare) ::

  This command creates a bare clone of an existing repository.
  The repository and the target directory are read from the user.

- Key: C m (mercit-clone-mirror) ::
...
- Key: C d (mercit-clone-shallow-since) ::
...
- Key: C e (mercit-clone-shallow-exclude) ::
...
- User Option: mercit-clone-set-remote-head ::
...
- User Option: mercit-clone-set-remote.pushDefault ::
...
- User Option: mercit-clone-default-directory ::
...
- User Option: mercit-clone-name-alist ::
  ...
  The format specified by option ~mercit-clone-url-format~ is used to
...
- User Option: mercit-clone-url-format ::
...
- Key: U (mercit-unstage-all) ::

  Remove all changes from the staging area.

- User Option: mercit-unstage-committed ::

  This option controls whether ~mercit-unstage~ "unstages" committed
  changes by reversing them in the index but not the working tree.
  The alternative is to raise an error.

- Key: M-x mercit-reverse-in-index ::
  ...
  command, but it is indirectly called when ~u~ (~mercit-unstage~) is
...
- Key: c e (mercit-commit-extend) ::
  ...
  ~mercit-commit-extend-override-date~ can be used to inverse the meaning
  of the prefix argument.

  Non-interactively respect the optional OVERRIDE-DATE argument and
  ignore the option.

- Key: c w (mercit-commit-reword) ::

  Reword the last commit, ignoring staged changes.  With a prefix
  argument keep the committer date, otherwise change it.  The option
  ~mercit-commit-reword-override-date~ can be used to inverse the meaning
  of the prefix argument.

  Non-interactively respect the optional OVERRIDE-DATE argument and
  ignore the option.

- Key: c f (mercit-commit-fixup) ::
...
- Key: c F (mercit-commit-instant-fixup) ::

  Create a fixup commit and instantly rebase.

- Key: c s (mercit-commit-squash) ::
...
- Key: c S (mercit-commit-instant-squash) ::

  Create a squash commit and instantly rebase.

- Key: c A (mercit-commit-augment) ::
...
- User Option: mercit-commit-diff-inhibit-same-window ::
...
~mercit-commit-diff~ from ~server-switch-hook~.
...
- Key: C-c C-w (mercit-pop-revision-stack) ::
  ...
  the ~mercit-revision-stack~ and inserts it into the current buffer
  according to ~mercit-pop-revision-stack-format~.  Revisions can be put
  on the stack using ~mercit-copy-section-value~ and
  ~mercit-copy-buffer-revision~.
...
- User Option: mercit-pop-revision-stack-format ::

  This option controls how the command ~mercit-pop-revision-stack~
...
- Function: git-commit-propertize-diff ::
  ...
  ~--verbose~ argument is used.  ~mercit-commit~ by default does not offer
...
- User Option: git-commit-post-finish-hook ::
  ...
  at all.  For certain commands such as ~mercit-rebase-continue~ this
...
changed using the command ~mercit-branch-configure~, which is available
...
The transient prefix command ~mercit-branch~ is used to create and
...
- User Option: mercit-branch-direct-configure ::

  This option controls whether the transient command ~mercit-branch~ can
  ...
  command ~mercit-branch-configure~ has to be used instead to view and
  change branch related variables.

- Key: b C (mercit-branch-configure) ::
- Key: f C ::
- Key: F C ::
- Key: P C ::
  ...
  a suffix of ~mercit-branch~ and on the ~mercit-branch-direct-configure~
  option.  If ~mercit-branch~ already displays the variables for the
...
- Key: b n (mercit-branch-create) ::
  ...
  Also see option ~mercit-branch-prefer-remote-upstream~.

- Key: b c (mercit-branch-and-checkout) ::

  This command creates a new branch like ~mercit-branch-create~, but then
  also checks it out.

  Also see option ~mercit-branch-prefer-remote-upstream~.

- Key: b l (mercit-branch-checkout) ::
  ...
  of ~mercit-branch-adjust-remote-upstream-alist~.

- Key: b s (mercit-branch-spinoff) ::
  ...
  ~mercit-branch-prefer-remote-upstream~ (which see), then the current
...
- Key: b S (mercit-branch-spinout) ::

  This command behaves like ~mercit-branch-spinoff~, except that it does
  not change the current branch.  If there are any uncommitted changes,
  then it behaves exactly like ~mercit-branch-spinoff~.

- Key: b x (mercit-branch-reset) ::
...
- User Option: mercit-branch-read-upstream-first ::
...
- User Option: mercit-branch-prefer-remote-upstream ::
...
- User Option: mercit-branch-adjust-remote-upstream-alist ::
...
- Command: mercit-branch-orphan ::

  This command creates and checks out a new orphan branch with
  contents from a given revision.

- Command: mercit-branch-or-checkout ::

  This command is a hybrid between ~mercit-checkout~ and
  ~mercit-branch-and-checkout~ and is intended as a replacement for the
  former in ~mercit-branch~.
  ...
  checks that out, just like ~mercit-checkout~ would.
  ...
  branch.  This is similar to what ~mercit-branch-and-checkout~ does.

  To use this command instead of ~mercit-checkout~ add this to your init
  file:

  #+BEGIN_SRC emacs-lisp
    (transient-replace-suffix 'mercit-branch 'mercit-checkout
      '("b" "dwim" mercit-branch-or-checkout))
  #+END_SRC

*** Branch Git Variables

These variables can be set from the transient prefix command
~mercit-branch-configure~.  By default they can also be set from
~mercit-branch~.  See [[*Branch Commands]].
...
- User Option: mercit-prefer-remote-upstream ::
  ...
  This affects all commands that use ~mercit-read-upstream-branch~ or
  ~mercit-read-starting-point~, which includes all commands that change
  the upstream and many which create new branches.

*** Auxiliary Branch Commands

These commands are not available from the transient ~mercit-branch~ by
default.

- Command: mercit-branch-shelve ::
...
- Command: mercit-branch-unshelve ::

  This command unshelves a branch that was previously shelved using
  ~mercit-branch-shelve~.  This is done by deleting the reference
...
- Key: m a (mercit-merge-absorb) ::
  ...
  Finally, if ~mercit-branch-pull-request~ was used to create the merged
  branch, then the respective remote branch is also removed.

- Key: m i (mercit-merge-into) ::
...
Additionally ~k~ (~mercit-discard~) on a hunk with unresolved conflicts
...
- Key: r p (mercit-rebase-onto-pushremote) ::
...
- Key: r u (mercit-rebase-onto-upstream) ::
...
- Key: r r (mercit-rebase-continue) ::
...
- Key: A A (mercit-sequence-continue) ::

  Resume the current cherry-pick or revert sequence.

- Key: A s (mercit-sequence-skip) ::

  Skip the stopped at commit during a cherry-pick or revert sequence.

- Key: A a (mercit-sequence-abort) ::
...
- Key: V V (mercit-revert-and-commit) ::
...
- Key: V v (mercit-revert-no-commit) ::
...
- Key: V A (mercit-sequence-continue) ::

  Resume the current cherry-pick or revert sequence.

- Key: V s (mercit-sequence-skip) ::

  Skip the stopped at commit during a cherry-pick or revert sequence.

- Key: V a (mercit-sequence-abort) ::
...
- Key: X m (mercit-reset-mixed) ::

  Reset the ~HEAD~ and index to some commit read from the user and
  defaulting to the commit at point.  The working tree is kept as-is.

- Key: X s (mercit-reset-soft) ::
...
- Key: z x (mercit-stash-keep-index) ::
...
The transient prefix command ~mercit-remote~ is used to add remotes and
...
- User Option: mercit-remote-direct-configure ::

  This option controls whether remote-related Git variables are
  accessible directly from the transient ~mercit-remote~.

  If ~t~ (the default) and a local branch is checked out, then
  ~mercit-remote~ features the variables for the upstream remote of that
  branch, or if ~HEAD~ is detached, for ~origin~, provided that exists.

  If ~nil~, then ~mercit-remote-configure~ has to be used to do so.

- Key: M C (mercit-remote-configure) ::
  ...
  Without a prefix argument this depends on whether it was invoked as
  a suffix of ~mercit-remote~ and on the ~mercit-remote-direct-configure~
  option.  If ~mercit-remote~ already displays the variables for the
...
- User Option: mercit-remote-add-set-remote.pushDefault ::
...
These variables can be set from the transient prefix command
~mercit-remote-configure~.  By default they can also be set from
~mercit-remote~.  See [[*Remote Commands]].
...
- Key: f p (mercit-fetch-from-pushremote) ::
...
- Key: f u (mercit-fetch-from-upstream) ::
...
- User Option: mercit-pull-or-fetch ::
  ...
  of the above suffix commands to the ~mercit-pull~ transient.

  If you do that, then you might also want to change the key binding
  for these prefix commands, e.g.:

  #+BEGIN_SRC emacs-lisp
    (setq mercit-pull-or-fetch t)
    (define-key mercit-mode-map "f" 'mercit-pull) ; was mercit-fetch
    (define-key mercit-mode-map "F" nil)         ; was mercit-pull
  #+END_SRC
...
- Key: F p (mercit-pull-from-pushremote) ::
...
- Key: F u (mercit-pull-from-upstream) ::
...
- Key: P p (mercit-push-current-to-pushremote) ::
...
- Key: P u (mercit-push-current-to-upstream) ::
...
- Command: mercit-push-implicitly args ::
  ...
  #+BEGIN_SRC emacs-lisp
    (transient-insert-suffix 'mercit-push \"p\"
      '(\"i\" mercit-push-implicitly))"
  #+END_SRC

- Command: mercit-push-to-remote remote args ::
...
- Key: w w (mercit-am-apply-patches) ::
...
- Key: w m (mercit-am-apply-maildir) ::
...
- Key: t r (mercit-tag-release) ::
  ...
  ~v1.2.3-custom.1~), you can set the ~mercit-release-tag-regexp~ and
  ~mercit-tag-version-regexp-alist~ variables.
...
- Key: T c (mercit-notes-merge-commit) ::

  Commit the current notes ref merge, after manually resolving
  conflicts.

- Key: T a (mercit-notes-merge-abort) ::

  Abort the current notes ref merge.

The following variables control what notes reference ~mercit-notes-*~,
...
The command ~mercit-list-submodules~ displays a list of the current
...
the super-repository by adding ~mercit-insert-modules~ to the hook
~mercit-status-sections-hook~ as described in [[*Status Module Sections]].

- Command: mercit-list-submodules ::
...
- User Option: mercit-submodule-list-columns ::

  This option controls what columns are displayed by the command
  ~mercit-list-submodules~ and how they are displayed.
...
- Key: o r (mercit-submodule-register) ::
...
- Key: Z (mercit-worktree) ::

  This transient prefix command binds the following suffix commands
  and displays them in a temporary buffer until a suffix is invoked.

- Key: Z b (mercit-worktree-checkout) ::

  Checkout BRANCH in a new worktree at PATH.

- Key: Z c (mercit-worktree-branch) ::

  Create a new BRANCH and check it out in a new worktree at PATH.

- Key: Z m (mercit-worktree-move) ::

  Move an existing worktree to a new PATH.

- Key: Z k (mercit-worktree-delete) ::

  Delete a worktree, defaulting to the worktree at point.
  The primary worktree cannot be deleted.

- Key: Z g (mercit-worktree-status) ::
...
- Key: > (mercit-sparse-checkout) ::

  This transient prefix command binds the following suffix commands
  and displays them in a temporary buffer until a suffix is invoked.

- Key: > e (mercit-sparse-checkout-enable) ::

  This command initializes a sparse checkout that includes only the
  files in the top-level directory.

  Note that ~mercit-sparse-checkout-set~ and
  ~mercit-sparse-checkout-add~ automatically initialize a sparse
  checkout if necessary.  However, you may want to call
  ~mercit-sparse-checkout-enable~ explicitly to re-initialize a sparse
  checkout after calling ~mercit-sparse-checkout-disable~, to pass
  additional arguments to ~git sparse-checkout init~, or to execute
  the initialization asynchronously.

- Key: > s (mercit-sparse-checkout-set) ::
...
- Key: > a (mercit-sparse-checkout-add) ::

  This command is like ~mercit-sparse-checkout-set~, but instead adds
  the specified list of directories to the set of directories that is
  already included in the sparse checkout.

- Key: > r (mercit-sparse-checkout-reapply) ::
...
- Command: mercit-switch-to-repository-buffer ::
+ Command: mercit-switch-to-repository-buffer-other-window ::
+ Command: mercit-switch-to-repository-buffer-other-frame ::
+ Command: mercit-display-repository-buffer ::
  ...
  The last variant uses ~mercit-display-buffer~ to do so and thus
  respects ~mercit-display-buffer-function~.

These are some of the commands that can be used in all buffers whose
major-modes derive from ~mercit-mode~.  There are other common commands
beside the ones below, but these didn't fit well anywhere else.

- Key: C-w (mercit-copy-section-value) ::
...
- Key: M-w (mercit-copy-buffer-revision) ::

  This command saves the revision being displayed in the current buffer
  to the ~kill-ring~ and also pushes it to the ~mercit-revision-stack~.  It
  is mainly intended for use in ~mercit-revision-mode~ buffers, the only
...
- User Option: mercit-wip-mode ::
  ...
  ~mercit-wip-*~ modes, which can also be used individually, if you want
...
~mercit-wip-log~ and ~mercit-wip-log-current~.  You should use ~--graph~ when
using these commands.

- Command: mercit-wip-log ::
  ...
  value of ~mercit-wip-merge-branch~ is ~nil~.

- Command: mercit-wip-log-current ::
...
Note that even if you enable ~mercit-wip-mode~ this won't give you
perfect protection.  The most likely scenario for losing changes
despite the use of ~mercit-wip-mode~ is making a change outside Emacs and
...
- User Option: mercit-wip-namespace ::
...
- User Option: mercit-wip-mode-lighter ::

  Mode-line lighter for ~mercit-wip--mode~.

*** Wip Graph

- User Option: mercit-wip-merge-branch ::
...
When ~mercit-wip-merge-branch~ is ~t~, then the history looks like this:
...
When ~mercit-wip-merge-branch~ is ~nil~, then creating a commit on the real
...
It is recommended that you use the mode ~mercit-wip-mode~ (which see) and
...
- User Option: mercit-wip-after-save-mode ::
...
- User Option: mercit-wip-after-apply-mode ::
...
- User Option: mercit-wip-before-change-mode ::
...
- User Option: mercit-wip-initial-backup-mode ::
...
- User Option: mercit-wip-after-save-local-mode-lighter ::

  Mode-line lighter for ~mercit-wip-after-save-local-mode~.

- User Option: mercit-wip-after-apply-mode-lighter ::

  Mode-line lighter for ~mercit-wip-after-apply-mode~.

- User Option: mercit-wip-before-change-mode-lighter ::

  Mode-line lighter for ~mercit-wip-before-change-mode~.

- User Option: mercit-wip-initial-backup-mode-lighter ::

  Mode-line lighter for ~mercit-wip-initial-backup-mode~.

** Commands for Buffers Visiting Files

Magit defines a few global key bindings unless the user sets
~mercit-define-global-key-bindings~ to ~nil~.  This includes binding ~C-c
M-g~ to ~mercit-file-dispatch~.  ~C-c g~ would be a much better binding
...
#+begin_src emacs-lisp
  (global-set-key (kbd "C-c g") 'mercit-file-dispatch)
#+end_src

The key bindings shown below assume that you have not improved the
binding for ~mercit-file-dispatch~.

- Key: C-c M-g (mercit-file-dispatch) ::

  This transient prefix command binds the following suffix commands
  and displays them in a temporary buffer until a suffix is invoked.

  When invoked in a buffer that does not visit a file, then it falls
  back to regular ~mercit-dispatch~.

- Key: C-c M-g s (mercit-stage-file) ::

  Stage all changes to the file being visited in the current buffer.

- Key: C-c M-g u (mercit-unstage-file) ::

  Unstage all changes to the file being visited in the current buffer.

- Key: C-c M-g c (mercit-commit) ::
...
- Key: C-c M-g d (mercit-diff-buffer-file) ::

  This command shows the diff for the file of blob that the current
  buffer visits.

- User Option: mercit-diff-buffer-file-locked ::

  This option controls whether ~mercit-diff-buffer-file~ uses a dedicated
  buffer.  See [[*Modes and Buffers]].

- Key: C-c M-g L (mercit-log) ::
...
- Key: C-c M-g l (mercit-log-buffer-file) ::
...
- Key: C-c M-g t (mercit-log-trace-definition) ::

  This command shows the log for the definition at point.

- User Option: mercit-log-buffer-file-locked ::

  This option controls whether ~mercit-log-buffer-file~ uses a dedicated
  buffer.  See [[*Modes and Buffers]].
...
- Key: C-c M-g p (mercit-blob-previous) ::
...
The ~mercit-blob-mode~ enables certain Magit features in blob-visiting
buffers.  Such buffers can be created using ~mercit-find-file~ and some
...
- Key: p (mercit-blob-previous) ::

  Visit the previous blob which modified the current file.

- Key: n (mercit-blob-next) ::

  Visit the next blob which modified the current file.

- Key: q (mercit-kill-this-buffer) ::
...
- Certain settings like ~fetch.prune=true~ are respected by Magit
  ...
  In this case the ~--prune~ argument in ~mercit-fetch~ might be active or
...
- ~/path/to/huge/repo/.dir-locals.el~
  ...
  #+BEGIN_SRC emacs-lisp
    ((nil . ((mercit-refresh-buffers . nil))))
  #+END_SRC
...
- ~/path/to/tag/invested/repo/.dir-locals.el~
  ...
  #+BEGIN_SRC emacs-lisp
    ((mercit-status-mode
      . ((eval . (mercit-disable-section-inserter 'mercit-insert-tags-header)))))
  #+END_SRC

# new list
- Function: mercit-disable-section-inserter fn ::
...
disabled by adding a symbol to ~mercit-no-confirm~ (see [[*Completion and
...
can recover it from there.  Option ~mercit-delete-by-moving-to-trash~
controls whether the system trash is used, which is the case by default.
Nevertheless, ~trash~ isn't a member of ~mercit-no-confirm~ - you
...
#+begin_src emacs-lisp
  (setq mercit-refresh-status-buffer nil)
#+end_src

You should also check whether any third-party packages have added
anything to ~mercit-refresh-buffer-hook~, ~mercit-status-refresh-hook~,
~mercit-pre-refresh-hook~, and ~mercit-post-refresh-hook~.  If so, then
check whether those additions impact performance significantly.

Magit can be told to refresh buffers verbosely using ~M-x
mercit-toggle-verbose-refresh~.  Enabling this helps figuring out which
...
~mercit-insert-tags-headers~, as explained at the mentioned node.
...
~mercit-diff-highlight-indentation~, ~mercit-diff-highlight-trailing~,
~mercit-diff-paint-whitespace~, ~mercit-diff-highlight-hunk-body~, and
~mercit-diff-refine-hunk~.
...
a commit using ~mercit-revision-mode~ takes considerably more time than
looking at the same commit in ~mercit-diff-mode~, then consider setting
~mercit-revision-insert-related-refs~ to ~nil~.
...
#+begin_src emacs-lisp
  (remove-hook 'mercit-refs-sections-hook 'mercit-insert-tags)
#+end_src
...
#+begin_src emacs-lisp
  (remove-hook 'server-switch-hook 'mercit-commit-diff)
  (remove-hook 'with-editor-filter-visit-hook 'mercit-commit-diff)
#+end_src
...
- User Option: mercit-define-global-key-bindings ::
...
https://github.com/mercit/mercit/wiki/Plugin-Dispatch-Key-Registry.

** Calling Git

Magit provides many specialized functions for calling Git.  All of
these functions are defined in either ~mercit-git.el~ or ~mercit-process.el~
and have one of the prefixes ~mercit-run-~, ~mercit-call-~, ~mercit-start-~,
or ~mercit-git-~ (which is also used for other things).
...
- Function: mercit-process-git destination &rest args ::
...
- Function: mercit-process-file process &optional infile buffer display &rest args ::
...
- User Option: mercit-git-debug ::

  Whether to report errors that occur when using ~mercit-git-insert~,
  ~mercit-git-string~, ~mercit-git-lines~, or ~mercit-git-items~.  This does
...
- Function: mercit-git-str &rest args ::

  This is a variant of ~mercit-git-string~ that ignores the option
  ~mercit-git-debug~.  It is mainly intended to be used while handling
...
- Function: mercit-call-git &rest args ::

  Calls git synchronously with ARGS.

- Function: mercit-call-process program &rest args ::

  Calls PROGRAM synchronously with ARGS.

- Function: mercit-run-git &rest args ::

  Calls git synchronously with ARGS and then refreshes.

- Function: mercit-run-git-with-input &rest args ::
...
- Function: mercit-run-git-async &rest args ::
  ...
  repository are reverted if ~mercit-revert-buffers~ is non-nil.

- Function: mercit-run-git-with-editor &rest args ::
...
- Function: mercit-start-git input &rest args ::
  ...
  Option ~mercit-git-executable~ specifies the Git executable and option
  ~mercit-git-global-arguments~ specifies constant arguments.  The
...
- Function: mercit-start-process &rest args ::
  ...
  use the sentinel ~mercit-process-sentinel~ and the filter
  ~mercit-process-filter~.  Information required by these functions is
  ...
  buffer that was current when ~mercit-start-process~ was called (if it
  ...
  in the current repository are reverted if ~mercit-revert-buffers~ is
  non-nil.

- Variable: mercit-this-process ::

  The child process which is about to start.  This can be used to
  change the filter and sentinel.

- Variable: mercit-process-raise-error ::

  When this is non-nil, then ~mercit-process-sentinel~ raises an error if
  git exits with a non-zero exit status.  For debugging purposes.

** Section Plumbing
*** Creating Sections

- Macro: mercit-insert-section &rest args ::
  ...
  a variable ~mercit-TYPE-section-map~ exists, then use that as the
  ...
  ~mercit-section-set-visibility-hook~.  When a section is recreated
  ...
  ~mercit-cancel-section~ can be used to abort and remove all traces of
...
- Function: mercit-insert-heading &rest args ::
  ...
  ~mercit-section-heading~ face for all inserted text.
...
- Function: mercit-cancel-section ::

  Cancel the section currently being inserted.  This exits the
  innermost call to ~mercit-insert-section~ and removes all traces of
  what has already happened inside that call.

- Function: mercit-define-section-jumper sym title &optional value ::
...
- Function: mercit-current-section ::

  Return the section at point.

- Function: mercit-region-sections &optional condition multiple ::
...
- Function: mercit-region-values &optional condition multiple ::
...
- Key: M-x mercit-describe-section-briefly ::

  Show information about the section at point.  This command is
  intended for debugging purposes.

- Function: mercit-section-ident section ::

  Return an unique identifier for SECTION.  The return value has the
  form ~((TYPE . VALUE)...)~.

- Function: mercit-get-section ident &optional root ::

  Return the section identified by IDENT.  IDENT has to be a list as
  returned by ~mercit-section-ident~.

- Function: mercit-section-match condition &optional section ::
...
- Function: mercit-section-value-if condition &optional section ::
...
- Function: mercit-section-case &rest clauses ::
...
- Variable: mercit-root-section ::
  ...
  ~mercit-insert-section~ and you should never modify it.

For diff related sections a few additional tools exist.

- Function: mercit-diff-type &optional section ::
  ...
  corresponding ~mercit-section~ struct) but takes additional information
  ...
  ~mercit-diff-mode~ SECTION is ignored and the type is determined using
  other means.  In ~mercit-revision-mode~ buffers the type is always
  ~committed~.

- Function: mercit-diff-scope &optional section strict ::
  ...
  confuse this with the diff "type", as returned by ~mercit-diff-type~.
...
displayed in an existing buffer do so by calling ~mercit-mode-setup~.
Among other things, that function sets the buffer local values of
~default-directory~ (to the top-level of the repository),
~mercit-refresh-function~, and ~mercit-refresh-args~.

Buffers are refreshed by calling the function that is the local value
of ~mercit-refresh-function~ (a function named ~mercit-*-refresh-buffer~,
where ~*~ may be something like ~diff~) with the value of
~mercit-refresh-args~ as arguments.

- Macro: mercit-mode-setup buffer switch-func mode refresh-func &optional refresh-args ::
  ...
  ~mercit-mode-display-buffer~ with BUFFER, MODE and SWITCH-FUNC as
  arguments.  Then it sets the local value of ~mercit-refresh-function~
  to REFRESH-FUNC and that of ~mercit-refresh-args~ to REFRESH-ARGS.
...
- Function: mercit-mode-display-buffer buffer mode &optional switch-function ::
  ...
  can later be restored by ~mercit-mode-bury-buffer~.
  ...
  derives from ~mercit-mode~.  Otherwise ~switch-to-buffer~ is used.

- Variable: mercit-refresh-function ::

  The value of this buffer-local variable is the function used to
  refresh the current buffer.  It is called with ~mercit-refresh-args~ as
  arguments.

- Variable: mercit-refresh-args ::

  The list of arguments used by ~mercit-refresh-function~ to refresh the
  current buffer.  ~mercit-refresh-function~ is called with these
  arguments.

  The value is usually set using ~mercit-mode-setup~, but in some cases
  it's also useful to provide commands that can change the value.  For
  example, the ~mercit-diff-refresh~ transient can be used to change any
  of the arguments used to display the diff, without having to specify
  again which differences should be shown, but ~mercit-diff-more-context~,
  ~mercit-diff-less-context~ and ~mercit-diff-default-context~ change just
  the ~-U<N>~ argument.  In both case this is done by changing the value
  of this variable and then calling this ~mercit-refresh-function~.
...
special care to adjust ~mercit-branch-current~ accordingly.  By default
it looks mostly like ~mercit-branch-local~, but with a box (by default
...
enough yet to make it valid or because she wants to use a non-mercit
...
"highlight" in their names.  For most sections, ~mercit-section-highlight~
...
between hunks.  That face ~mercit-diff-hunk-heading~, should be different
from both ~mercit-diff-hunk-heading-highlight~ and
~mercit-section-highlight~, as well as from ~mercit-diff-context~ and
~mercit-diff-context-highlight~.  By default we do that by changing the
...
instead use one of the faces ~mercit-diff-file-heading-selection~ or
~mercit-diff-hunk-heading-selection~.  These faces have to be different
...
~mercit-diff-refine-hunk~ to ~all~.  You might personally prefer to only
...
(Also turn on ~mercit-diff-highlight-indentation~,
~mercit-diff-highlight-trailing~, and ~mercit-diff-paint-whitespace~; and
insert some whitespace errors into the code you use for testing.)

For added lines you have to adjust three faces:
~mercit-diff-added~, ~mercit-diff-added-highlight~, and
...
hunk heading is shown using ~mercit-diff-hunk-heading-selection~, and so
...
https://github.com/mercit/mercit/wiki/FAQ.
...
Also see https://mercit.vc/assets/videos/magic.mp4.
...
isn't available, ~M-x mercit-process-buffer~).  This will show a buffer
...
that approach, then set the value of ~mercit-view-git-manual-method~ to
one of the supported packages ~man~ or ~woman~, e.g.:

#+begin_src emacs-lisp
  (setq mercit-view-git-manual-method 'man)
#+end_src
...
Please see [[*Branching]] and http://emacsair.me/2016/01/17/mercit-2.4
...
https://github.com/mercit/mercit/wiki/Pushing-with-Magit-from-Windows.
...
#+begin_src emacs-lisp
  (load "/path/to/mercit-autoloads.el")
#+end_src
...
instead use ~mercit-diff-visit-file-hook~ to reveal the text, possibly
...
https://github.com/mercit/mercit/issues/3624.
...
https://mercit.vc/manual/transient/Saving-Values.html#Saving-Values for
more details.
...
- Key: M-x mercit-emacs-Q-command ::
  ...
  This shell command starts ~emacs~ with only ~mercit~ and its
...
- Key: M-x mercit-toggle-git-debug ::
...
- Key: M-x mercit-toggle-verbose-refresh ::
...
#  LocalWords:  logfile mercit maildir manpage manpages minibuffer multi mv
...
# Local Variables:
# eval: (require 'mercit-base nil t)
