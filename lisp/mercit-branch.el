;;; mercit-branch.el --- Branch support  -*- lexical-binding:t -*-
...
(defcustom mercit-branch-read-upstream-first t
...
(defcustom mercit-branch-prefer-remote-upstream nil
...
(defcustom mercit-branch-adjust-remote-upstream-alist nil
...
(defcustom mercit-branch-rename-push-target t
  "Whether the push-remote setup is preserved when renaming a branch.

The command `mercit-branch-rename' renames a branch named OLD to
...
;;;###autoload (autoload 'mercit-branch "mercit" nil t)
(transient-define-prefix mercit-branch (branch)
  "Add, configure or remove a branch."
  :man-page "git-branch"
  ["Arguments"
   (7 "-r" "Recurse submodules when checking out an existing branch"
      "--recurse-submodules"
      :if (lambda () (mercit-git-version>= "2.13")))]
  ["Variables"
   :if (lambda ()
         (and mercit-branch-direct-configure
              (oref transient--prefix scope)))
   ("d" mercit-branch.<branch>.description)
   ("u" mercit-branch.<branch>.merge/remote)
...
(defun mercit-branch-arguments ()
  (transient-args 'mercit-branch))

;;;###autoload
(defun mercit-checkout (revision &optional args)
...
\n(git checkout REVISION)."
  (interactive (list (mercit-read-other-branch-or-commit "Checkout")
                     (mercit-branch-arguments)))
  (when (string-match "\\`heads/\\(.+\\)" revision)
    (setq revision (match-string 1 revision)))
  (mercit-run-git "checkout" args revision))

;;;###autoload
(defun mercit-branch-create (branch start-point)
  "Create BRANCH at branch or revision START-POINT."
  (interactive (mercit-branch-read-args "Create branch"))
  (mercit-call-git "branch" branch start-point)
  (mercit-branch-maybe-adjust-upstream branch start-point)
  (mercit-refresh))

;;;###autoload
(defun mercit-branch-and-checkout (branch start-point &optional args)
  "Create and checkout BRANCH at branch or revision START-POINT."
  (interactive (append (mercit-branch-read-args "Create and checkout branch")
                       (list (mercit-branch-arguments))))
  (if (string-match-p "^stash@{[0-9]+}$" start-point)
      (mercit-run-git "stash" "branch" branch start-point)
    (mercit-call-git "checkout" args "-b" branch start-point)
    (mercit-branch-maybe-adjust-upstream branch start-point)
    (mercit-refresh)))

;;;###autoload
(defun mercit-branch-or-checkout (arg &optional start-point)
  "Hybrid between `mercit-checkout' and `mercit-branch-and-checkout'.

Ask the user for an existing branch or revision.  If the user
input actually can be resolved as a branch or revision, then
check that out, just like `mercit-checkout' would.

Otherwise create and checkout a new branch using the input as
its name.  Before doing so read the starting-point for the new
branch.  This is similar to what `mercit-branch-and-checkout'
does."
  (interactive
   (let ((arg (mercit-read-other-branch-or-commit "Checkout")))
     (list arg
           (and (not (mercit-commit-p arg))
                (mercit-read-starting-point "Create and checkout branch" arg)))))
  (when (string-match "\\`heads/\\(.+\\)" arg)
    (setq arg (match-string 1 arg)))
  (if start-point
      (mercit-branch-and-checkout arg start-point)
    (mercit-checkout arg)))

;;;###autoload
(defun mercit-branch-checkout (branch &optional start-point)
...
value of `mercit-branch-adjust-remote-upstream-alist', just like
when using `mercit-branch-and-checkout'."
  (interactive
   (let* ((current (mercit-get-current-branch))
          (local   (mercit-list-local-branch-names))
     ...
     (cond ((member choice remote)
           ...
           ((member choice local)
            (list choice))
           (t
            (list choice (mercit-read-starting-point "Create" choice))))))
  (if (not start-point)
      (mercit-checkout branch (mercit-branch-arguments))
    (when (mercit-anything-modified-p t)
      (user-error "Cannot checkout when there are uncommitted changes"))
    (let ((mercit-inhibit-refresh t))
      (mercit-branch-and-checkout branch start-point))
    (when (mercit-remote-branch-p start-point)
      (pcase-let ((`(,remote . ,remote-branch)
                   (mercit-split-branch-name start-point)))
        (when (and (equal branch remote-branch)
                   (not (equal remote (mercit-get "remote.pushDefault"))))
          (mercit-set remote "branch" branch "pushRemote"))))
    (mercit-refresh)))

(defun mercit-branch-maybe-adjust-upstream (branch start-point)
  (--when-let
      (or (and (mercit-get-upstream-branch branch)
               (mercit-get-indirect-upstream-branch start-point))
          (and (mercit-remote-branch-p start-point)
               (let ((name (cdr (mercit-split-branch-name start-point))))
                 (-some (pcase-lambda (`(,upstream . ,rule))
                          (and (mercit-branch-p upstream)
                               (if (listp rule)
                                   (not (member name rule))
                                 (string-match-p rule name))
                               upstream))
                        mercit-branch-adjust-remote-upstream-alist))))
    (mercit-call-git "branch" (concat "--set-upstream-to=" it) branch)))

;;;###autoload
(defun mercit-branch-orphan (branch start-point)
  "Create and checkout an orphan BRANCH with contents from revision START-POINT."
  (interactive (mercit-branch-read-args "Create and checkout orphan branch"))
  (mercit-run-git "checkout" "--orphan" branch start-point))

(defun mercit-branch-read-args (prompt &optional default-start)
  (if mercit-branch-read-upstream-first
      (let ((choice (mercit-read-starting-point prompt nil default-start)))
        (if (mercit-rev-verify choice)
            (list (mercit-read-string-ns
                   (if mercit-completing-read--silent-default
                       (format "%s (starting at `%s')" prompt choice)
                     "Name for new branch")
                   (let ((def (mapconcat #'identity
                                         (cdr (split-string choice "/"))
                                         "/")))
                     (and (member choice (mercit-list-remote-branch-names))
                          (not (member def (mercit-list-local-branch-names)))
                          def)))
                  choice)
          (if (eq mercit-branch-read-upstream-first 'fallback)
              (list choice
                    (mercit-read-starting-point prompt choice default-start))
            (user-error "Not a valid starting-point: %s" choice))))
    (let ((branch (mercit-read-string-ns (concat prompt " named"))))
      (list branch (mercit-read-starting-point prompt branch default-start)))))

;;;###autoload
(defun mercit-branch-spinout (branch &optional from)
  "Create new branch from the unpushed commits.
Like `mercit-branch-spinoff' but remain on the current branch.
If there are any uncommitted changes, then behave exactly like
`mercit-branch-spinoff'."
  (interactive (list (mercit-read-string-ns "Spin out branch")
                     (car (last (mercit-region-values 'commit)))))
  (mercit--branch-spinoff branch from nil))

;;;###autoload
(defun mercit-branch-spinoff (branch &optional from)
  "Create new branch from the unpushed commits.
...
`mercit-branch-prefer-remote-upstream' (which see), then the
...
from the source branch's upstream, then an error is raised."
  (interactive (list (mercit-read-string-ns "Spin off branch")
                     (car (last (mercit-region-values 'commit)))))
  (mercit--branch-spinoff branch from t))

(defun mercit--branch-spinoff (branch from checkout)
  (when (mercit-branch-p branch)
  ...
  (when (and (not checkout)
    ...
    (setq checkout t))
  (if-let ((current (mercit-get-current-branch)))
      (let ((tracked (mercit-get-upstream-branch current))
            base)
        (when from
          (unless (mercit-rev-ancestor-p from current)
            (user-error "Cannot spin off %s.  %s is not reachable from %s"
                        branch from current))
          (when (and tracked
                     (mercit-rev-ancestor-p from tracked))
            (user-error "Cannot spin off %s.  %s is ancestor of upstream %s"
                        branch from tracked)))
        (let ((mercit-process-raise-error t))
          (if checkout
              (mercit-call-git "checkout" "-b" branch current)
            (mercit-call-git "branch" branch current)))
        (--when-let (mercit-get-indirect-upstream-branch current)
          (mercit-call-git "branch" "--set-upstream-to" it branch))
        (when (and tracked
                   (setq base
                         (if from
                             (concat from "^")
                           (mercit-git-string "merge-base" current tracked)))
                   (not (mercit-rev-eq base current)))
          (if checkout
              (mercit-call-git "update-ref" "-m"
                              (format "reset: moving to %s" base)
                              (concat "refs/heads/" current) base)
            (mercit-call-git "reset" "--hard" base))))
    (if checkout
        (mercit-call-git "checkout" "-b" branch)
      (mercit-call-git "branch" branch)))
  (mercit-refresh))

;;;###autoload
(defun mercit-branch-reset (branch to &optional set-upstream)
...
that is being reset."
  (interactive
   (let* ((atpoint (mercit-local-branch-at-point))
          (branch  (mercit-read-local-branch "Reset branch" atpoint)))
     (list branch
           (mercit-completing-read (format "Reset %s to" branch)
                                  (delete branch (mercit-list-branch-names))
                                  nil nil nil 'mercit-revision-history
  ...
  (let ((mercit-inhibit-refresh t))
    (if (equal branch (mercit-get-current-branch))
        (if (and (mercit-anything-modified-p)
                 (not (yes-or-no-p
                       "Uncommitted changes will be lost.  Proceed? ")))
            (user-error "Abort")
          (mercit-reset-hard to))
      (mercit-call-git "update-ref"
                      "-m" (format "reset: moving to %s" to)
                      (mercit-git-string "rev-parse" "--symbolic-full-name"
                                        branch)
                      to))
    (when (and set-upstream (mercit-branch-p to))
      (mercit-set-upstream-branch branch to)
      (mercit-branch-maybe-adjust-upstream branch to)))
  (mercit-refresh))

(defvar mercit-branch-delete-never-verify nil
  "Whether `mercit-branch-delete' always pushes with \"--no-verify\".")

;;;###autoload
(defun mercit-branch-delete (branches &optional force)
  "Delete one or multiple branches.
If the region marks multiple branches, then offer to delete
those, otherwise prompt for a single branch to be deleted,
defaulting to the branch at point."
  ;; One would expect this to be a command as simple as, for example,
  ;; `mercit-branch-rename'; but it turns out everyone wants to squeeze
  ;; a bit of extra functionality into this one, including myself.
  (interactive
   (let ((branches (mercit-region-values 'branch t))
         (force current-prefix-arg))
     (if (length> branches 1)
         (mercit-confirm t nil "Delete %i branches" nil branches)
       (setq branches
             (list (mercit-read-branch-prefer-other
                    (if force "Force delete branch" "Delete branch")))))
     (unless force
       (when-let ((unmerged (-remove #'mercit-branch-merged-p branches)))
         (if (mercit-confirm 'delete-unmerged-branch
           ...
           (or (setq branches (-difference branches unmerged))
               (user-error "Abort")))))
     (list branches force)))
  (let* ((refs (mapcar #'mercit-ref-fullname branches))
    ...
    (when ambiguous
      (user-error
       ...
       (let ((len (length ambiguous)))
         (cond
          ((= len 1)
           (format "%s is" (-first #'mercit-ref-ambiguous-p branches)))
          ((= len (length refs))
    ...
    (cond
     ((string-match "^refs/remotes/\\([^/]+\\)" (car refs))
      (let* ((remote (match-string 1 (car refs)))
        ...
        (cond
         ((mercit-confirm 'delete-branch-on-remote
          ...
          (mercit-run-git-async
           "push"
           (and (or force mercit-branch-delete-never-verify) "--no-verify")
           remote
           (--map (concat ":" (substring it offset)) branches))
          ;; If that is not the case, then this deletes the tracking branches.
          (set-process-sentinel
           mercit-this-process
           (apply-partially #'mercit-delete-remote-branch-sentinel remote refs)))
         (t
          (dolist (ref refs)
            (message "Delete %s (was %s)" ref
                     (mercit-rev-parse "--short" ref))
          ...
          (mercit-refresh)))))
     ((length> branches 1)
      (setq branches (delete (mercit-get-current-branch) branches))
      (mapc #'mercit-branch-maybe-delete-pr-remote branches)
      (mapc #'mercit-branch-unset-pushRemote branches)
      (mercit-run-git "branch" (if force "-D" "-d") branches))
     (t ; And now for something completely different.
      (let* ((branch (car branches))
             (prompt (format "Branch %s is checked out.  " branch))
             (target (mercit-get-indirect-upstream-branch branch t)))
        (when (equal branch (mercit-get-current-branch))
          (when (or (equal branch target)
                    (not target))
            (setq target (mercit-main-branch)))
          (pcase (if (or (equal branch target)
            ...
            (`detach (unless (or (equal force '(4))
                       ...
                       (mercit-confirm 'delete-unmerged-branch
                         "Delete unmerged branch %s" ""
                         nil (list branch)))
                     (mercit-call-git "checkout" "--detach"))
            (`target (unless (or (equal force '(4))
                                 (member branch force)
                                 (mercit-branch-merged-p branch target))
                       (mercit-confirm 'delete-unmerged-branch
                         "Delete unmerged branch %s" ""
                         nil (list branch)))
                     (mercit-call-git "checkout" target))
            (`abort  (user-error "Abort")))
          (setq force t))
        (mercit-branch-maybe-delete-pr-remote branch)
        (mercit-branch-unset-pushRemote branch)
        (mercit-run-git "branch" (if force "-D" "-d") branch))))))

(put 'mercit-branch-delete 'interactive-only t)

(defun mercit-branch-maybe-delete-pr-remote (branch)
  (when-let ((remote (mercit-get "branch" branch "pullRequestRemote")))
    (let* ((variable (format "remote.%s.fetch" remote))
      ...
      (unless (member (format "+refs/heads/*:refs/remotes/%s/*" remote)
                      refspecs)
        (let ((refspec
               (if (equal (mercit-get "branch" branch "pushRemote") remote)
                   (format "+refs/heads/%s:refs/remotes/%s/%s"
                           branch remote branch)
                 (let ((merge (mercit-get "branch" branch "merge")))
                   (and merge
          ...
          (when (member refspec refspecs)
            (if (and (length= refspecs 1)
                     (mercit-confirm 'delete-pr-remote
                       (format "Also delete remote %s (%s)" remote
                               "no pull-request branch remains")
                       nil t))
                (mercit-call-git "remote" "rm" remote)
              (mercit-call-git "config" "--unset-all" variable
                              (format "^%s$" (regexp-quote refspec))))))))))

(defun mercit-branch-unset-pushRemote (branch)
  (mercit-set nil "branch" branch "pushRemote"))

(defun mercit-delete-remote-branch-sentinel (remote refs process event)
  (when (memq (process-status process) '(exit signal))
    (if (= (process-exit-status process) 1)
        (if-let ((on-remote (--map (concat "refs/remotes/" remote "/" it)
                                   (mercit-remote-list-branches remote)))
                 (rest (--filter (and (not (member it on-remote))
                                      (mercit-ref-exists-p it))
                                 refs)))
            (progn
...
;;;###autoload
(defun mercit-branch-rename (old new &optional force)
...
the value of `mercit-branch-rename-push-target' (which see) maybe
set `branch.NEW.pushRemote' and maybe rename the push-target on
the remote."
  (interactive
   (let ((branch (mercit-read-local-branch "Rename branch")))
     (list branch
           (mercit-read-string-ns (format "Rename branch '%s' to" branch)
                                 nil 'mercit-revision-history)
           current-prefix-arg)))
  (when (string-match "\\`heads/\\(.+\\)" old)
    (setq old (match-string 1 old)))
  (when (equal old new)
    (user-error "Old and new branch names are the same"))
  (mercit-call-git "branch" (if force "-M" "-m") old new)
  (when mercit-branch-rename-push-target
    (let ((remote (mercit-get-push-remote old))
          (old-specified (mercit-get "branch" old "pushRemote"))
          (new-specified (mercit-get "branch" new "pushRemote")))
      (when (and old-specified (or force (not new-specified)))
        ...
        ;; what we expect here...
        (mercit-set old-specified "branch" new "pushRemote"))
      (when (and (equal (mercit-get-push-remote new) remote)
                 ;; ...and if it does not, then we must abort.
                 (not (eq mercit-branch-rename-push-target 'local-only))
                 (or (not (memq mercit-branch-rename-push-target
                                '(forge-only github-only)))
                     (and (require (quote forge) nil t)
                          (fboundp 'forge--forge-remote-p)
                          (forge--forge-remote-p remote))))
        (let ((old-target (mercit-get-push-branch old t))
              (new-target (mercit-get-push-branch new t))
              (remote (mercit-get-push-remote new)))
          (when (and old-target
                     (not new-target)
                     (mercit-y-or-n-p (format "Also rename %S to %S on \"%s\""
                                             old new remote)))
            ...
            (mercit-call-git "push" "-v" remote
                            (format "%s:refs/heads/%s" old-target new)
                            (format ":refs/heads/%s" old)))))))
  (mercit-branch-unset-pushRemote old)
  (mercit-refresh))

;;;###autoload
(defun mercit-branch-shelve (branch)
  "Shelve a BRANCH.
Rename \"refs/heads/BRANCH\" to \"refs/shelved/BRANCH\",
and also rename the respective reflog file."
  (interactive (list (mercit-read-other-local-branch "Shelve branch")))
  (let ((old (concat "refs/heads/"   branch))
        (new (concat "refs/shelved/" branch)))
    (mercit-git "update-ref" new old "")
    (mercit--rename-reflog-file old new)
    (mercit-branch-unset-pushRemote branch)
    (mercit-run-git "branch" "-D" branch)))

;;;###autoload
(defun mercit-branch-unshelve (branch)
  "Unshelve a BRANCH
Rename \"refs/shelved/BRANCH\" to \"refs/heads/BRANCH\",
and also rename the respective reflog file."
  (interactive
   (list (mercit-completing-read
          "Unshelve branch"
          (--map (substring it 8)
                 (mercit-list-refnames "refs/shelved"))
          nil t)))
  (let ((old (concat "refs/shelved/" branch))
        (new (concat "refs/heads/"   branch)))
    (mercit-git "update-ref" new old "")
    (mercit--rename-reflog-file old new)
    (mercit-run-git "update-ref" "-d" old)))

(defun mercit--rename-reflog-file (old new)
  (let ((old (mercit-git-dir (concat "logs/" old)))
        (new (mercit-git-dir (concat "logs/" new))))
...
;;;###autoload (autoload 'mercit-branch-configure "mercit-branch" nil t)
(transient-define-prefix mercit-branch-configure (branch)
  "Configure a branch."
  :man-page "git-branch"
  [:description
   (lambda ()
     (concat
      ...
      (propertize (oref transient--prefix scope) 'face 'mercit-branch-local)))
   ("d"   mercit-branch.<branch>.description)
   ("u"   mercit-branch.<branch>.merge/remote)
   ("r"   mercit-branch.<branch>.rebase)
   ("p"   mercit-branch.<branch>.pushRemote)]
  ["Configure repository defaults"
   ("R" mercit-pull.rebase)
   ("P" mercit-remote.pushDefault)]
  ["Configure branch creation"
   ("a m" mercit-branch.autoSetupMerge)
   ("a r" mercit-branch.autoSetupRebase)]
  (interactive
   (list (or (and (not current-prefix-arg)
                  (not (and mercit-branch-direct-configure
                            (eq transient-current-command 'mercit-branch)))
                  (mercit-get-current-branch))
             (mercit--read-branch-scope))))
  (transient-setup 'mercit-branch-configure nil nil :scope branch))

(defun mercit--read-branch-scope (&optional obj)
  (mercit-read-local-branch
   (if obj
       (format "Set %s for branch"
               (format (oref obj variable) "<name>"))
     "Configure branch")))

(transient-define-suffix mercit-branch.<branch>.description (branch)
...
(add-hook 'find-file-hook #'mercit-branch-description-check-buffers)

(defun mercit-branch-description-check-buffers ()
  (and buffer-file-name
       (string-match-p "/\\(BRANCH\\|EDIT\\)_DESCRIPTION\\'" buffer-file-name)))

(defclass mercit--git-branch:upstream (mercit--git-variable)
  ((format :initform " %k %m %M\n   %r %R")))

(transient-define-infix mercit-branch.<branch>.merge/remote ()
  :class 'mercit--git-branch:upstream)

(cl-defmethod transient-init-value ((obj mercit--git-branch:upstream))
...
(cl-defmethod transient-infix-read ((obj mercit--git-branch:upstream))
  (if (oref obj value)
      (oset obj value nil)
    (mercit-read-upstream-branch (oref transient--prefix scope) "Upstream")))

(cl-defmethod transient-infix-set ((obj mercit--git-branch:upstream) refname)
  (mercit-set-upstream-branch (oref transient--prefix scope) refname)
  (oset obj value
        (and-let* ((branch (oref transient--prefix scope))
                   (r (mercit-get "branch" branch "remote"))
                   (m (mercit-get "branch" branch "merge")))
          (list r m)))
  (mercit-refresh))

(cl-defmethod transient-format ((obj mercit--git-branch:upstream))
...
(cl-defmethod transient-format-value ((obj mercit--git-branch:upstream) key)
...
;;; _
(provide 'mercit-branch)
;;; mercit-branch.el ends here
