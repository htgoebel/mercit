...
(defgroup mercit-auto-revert nil
...
is enabled."
  ...
  :type '(radio (const :tag "No filter" nil)
                (function-item mercit-auto-revert-buffer-p)
                (function-item mercit-auto-revert-repository-buffer-p)
                function))
...
(defcustom mercit-auto-revert-tracked-only t
  ...
  :set (lambda (var val)
         (set var val)
         (when (and (bound-and-true-p mercit-auto-revert-mode)
                    (featurep 'mercit-autorevert))
           (mercit-auto-revert-mode -1)
...
(defun mercit-turn-on-auto-revert-mode-if-desired (&optional file)
  (if file
    ...
    (when (and (not auto-revert-mode)        ; see #3014
               ...
               (compat-call executable-find (mercit-git-executable) t)
               (mercit-toplevel)
               (or (not mercit-auto-revert-tracked-only)
...
;; - If the user has set the variable `mercit-auto-revert-mode' to nil
;;   after loading mercit (instead of doing so before loading mercit or
;;   by using the function), then we should still respect that setting.
;; - If the user sets one of these variables after loading mercit and
;;   after `after-init-hook' has run, then that won't have an effect
;;   and there is nothing we can do about it.
(defun mercit-auto-revert-mode--init-kludge ()
...
and code surrounding the definition of this function."
  (if mercit-auto-revert-mode
      (let ((start (current-time)))
        (mercit-message "Turning on mercit-auto-revert-mode...")
        (mercit-auto-revert-mode 1)
        (mercit-message
         "Turning on mercit-auto-revert-mode...done%s"
...
(if after-init-time
    ...
    (mercit-auto-revert-mode--init-kludge)
...
previously untracked file using `mercit-stage', then this mode
notices that.
...
in the `autorevert' and `mercit-autorevert' groups.

This function calls the hook `mercit-auto-revert-mode-hook'.
...
(defun mercit-auto-revert-buffers ()
  (when (and mercit-auto-revert-immediately
             (or global-auto-revert-mode
                 (and mercit-auto-revert-mode auto-revert-buffer-list)))
    (let ((auto-revert-buffer-list-filter
           (or auto-revert-buffer-list-filter
               #'mercit-auto-revert-repository-buffer-p)))
      (auto-revert-buffers))))

(defvar mercit-auto-revert-toplevel nil)

(defvar mercit-auto-revert-counter 1
  "Incremented each time `auto-revert-buffers' is called.")

(defun mercit-auto-revert-buffer-p (buffer)
...
If there is no current repository, then return t for any BUFFER."
  (mercit-auto-revert-repository-buffer-p buffer t))

(defun mercit-auto-revert-repository-buffer-p (buffer &optional fallback)
  "Return non-nil if BUFFER visits a file inside the current repository.
The current repository is the one containing `default-directory'.
If there is no current repository, then return FALLBACK (which
defaults to nil) for any BUFFER."
  ;; Call `mercit-toplevel' just once per cycle.
  (unless (and mercit-auto-revert-toplevel
               (= (cdr mercit-auto-revert-toplevel)
                  mercit-auto-revert-counter))
    (setq mercit-auto-revert-toplevel
          (cons (or (mercit-toplevel) 'no-repo)
                mercit-auto-revert-counter)))
  (let ((top (car mercit-auto-revert-toplevel)))
    (if (eq top 'no-repo)
        fallback
...
(defun auto-revert-buffers--buffer-list-filter (fn)
  (cl-incf mercit-auto-revert-counter)
...
;;; _
(provide 'mercit-autorevert)
;;; mercit-autorevert.el ends here
