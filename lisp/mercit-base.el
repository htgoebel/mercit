...
(defconst mercit--minimal-git "2.2.0")
(defconst mercit--minimal-emacs "25.1")
...
(defcustom mercit-completing-read-function #'mercit-builtin-completing-read
  "Function to be called when requesting input from the user.

If you have enabled `ivy-mode' or `helm-mode', then you don't
have to customize this option; `mercit-builtin-completing-read'
...
alphabetical order, depending on your version of Ivy."
  ...
  :type '(radio (function-item mercit-builtin-completing-read)
                (function-item mercit-ido-completing-read)
                (function-item ivy-completing-read)
                (function-item helm--completing-read-default)
                (function :tag "Other function")))

(defcustom mercit-dwim-selection
  '((mercit-stash-apply        nil t)
    (mercit-ediff-resolve-all  nil t)
    (mercit-ediff-resolve-rest nil t)
    (mercit-stash-branch       nil t)
    (mercit-stash-branch-here  nil t)
    (mercit-stash-format-patch nil t)
    (mercit-stash-drop         nil ask)
    (mercit-stash-pop          nil ask)
...
- COMMAND is the command that should not prompt for a choice.
  ...
  `mercit-completing-read' or a utility function which in turn uses
  that function.

- If the command uses `mercit-completing-read' multiple times, then
  ...
  the PROMPT argument passed to `mercit-completing-read'.

- DEFAULT specifies how to use the default.  If it is t, then
  the DEFAULT argument passed to `mercit-completing-read' is used
...
(defconst mercit--confirm-actions
  '((const discard)
    (const reverse)
...
Files:
  ...
  Also see option `mercit-delete-by-moving-to-trash'.
...
References:
  ...
  remote, then `mercit-branch-delete' offers to delete the remote
...
Edit published history:
  ...
  branches listed in `mercit-published-branches'.
  ...
  `edit-published' Affects the commands `mercit-edit-line-commit'
  and `mercit-diff-edit-hunk-commit'.  These two commands make
  ...
  or set `mercit-published-branches' to nil.
...
Global settings:
  ...
  When `mercit-wip-before-change-mode' is enabled then these actions
...
(defcustom mercit-slow-confirm '(drop-stashes)
...
(defcustom mercit-ellipsis (if (char-displayable-p ?…) "…" "...")
  "String used to abbreviate text in process buffers.

Currently this is only used to elide `mercit-git-global-arguments'
...
(defcustom mercit-update-other-window-delay 0.2
...
(defclass mercit-commit-section (mercit-section) ())

(setf (alist-get 'commit mercit--section-type-alist) 'mercit-commit-section)

(defclass mercit-diff-section (mercit-section) () :abstract t)

(defclass mercit-file-section (mercit-diff-section)
  ((keymap :initform 'mercit-file-section-map)
   (source :initform nil)
   (header :initform nil)))

(defclass mercit-module-section (mercit-file-section)
  ((keymap :initform 'mercit-module-section-map)
   (range  :initform nil)))

(defclass mercit-hunk-section (mercit-diff-section)
  ((keymap      :initform 'mercit-hunk-section-map)
...
(setf (alist-get 'file   mercit--section-type-alist) 'mercit-file-section)
(setf (alist-get 'module mercit--section-type-alist) 'mercit-module-section)
(setf (alist-get 'hunk   mercit--section-type-alist) 'mercit-hunk-section)

(defclass mercit-log-section (mercit-section) () :abstract t)
(defclass mercit-unpulled-section (mercit-log-section) ())
(defclass mercit-unpushed-section (mercit-log-section) ())
(defclass mercit-unmerged-section (mercit-log-section) ())

(setf (alist-get 'unpulled mercit--section-type-alist) 'mercit-unpulled-section)
(setf (alist-get 'unpushed mercit--section-type-alist) 'mercit-unpushed-section)
(setf (alist-get 'unmerged mercit--section-type-alist) 'mercit-unmerged-section)
...
(defvar mercit-completing-read--silent-default nil)

(defun mercit-completing-read (prompt collection &optional
...
`mercit-dwim-selection'.  See the documentation of the latter for
more information.

If it does use the default without the user even having to
confirm that, then `mercit-completing-read--silent-default' is set
to t, otherwise nil.
...
- FALLBACK specifies a secondary default that is only used if
  ...
  subject to `mercit-dwim-selection' — if DEF is nil but FALLBACK
...
- PROMPT is modified to end with \" (default DEF|FALLBACK): \"
  ...
  `mercit-completing-read-function' is set to its default value of
  `mercit-builtin-completing-read'."
  (setq mercit-completing-read--silent-default nil)
  (if-let ((dwim (and def
                      (nth 2 (-first (pcase-lambda (`(,cmd ,re ,_))
                                       (and (eq this-command cmd)
                                            (or (not re)
                                                (string-match-p re prompt))))
                                     mercit-dwim-selection)))))
      (if (eq dwim 'ask)
          (if (y-or-n-p (format "%s %s? " prompt def))
              def
            (user-error "Abort"))
        (setq mercit-completing-read--silent-default t)
        def)
    (unless def
      (setq def fallback))
    (let ((command this-command)
          (reply (funcall mercit-completing-read-function
                          (concat prompt ": ")
...
(defun mercit--completion-table (collection)
...
(defun mercit-builtin-completing-read
  ...
  (unless (or (bound-and-true-p helm-mode)
    ...
    (setq prompt (mercit-prompt-with-default prompt def)))
  (unless (or (bound-and-true-p helm-mode)
              (bound-and-true-p ivy-mode))
    (setq choices (mercit--completion-table choices)))
  (cl-letf (((symbol-function #'completion-pcm--all-completions)))
    (when (< emacs-major-version 26)
      (fset 'completion-pcm--all-completions
            'mercit-completion-pcm--all-completions))
    (let ((ivy-sort-functions-alist nil))
      (completing-read prompt choices
                       predicate require-match
                       initial-input hist def))))

(defun mercit-completing-read-multiple
...
third-party completion frameworks."
  (cl-letf*
      (;; To implement NO-SPLIT we have to manipulate the respective
       ...
       (table (mercit--completion-table table))
...
(defun mercit-ido-completing-read
    (prompt choices &optional predicate require-match initial-input hist def)
...
(defun mercit-prompt-with-default (prompt def)
  (if (and def (length> prompt 2)
           (string-equal ": " (substring prompt -2)))
      (format "%s (default %s): " (substring prompt 0 -2) def)
    prompt))

(defvar mercit-minibuffer-local-ns-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map minibuffer-local-map)
    (define-key map "\s" #'mercit-whitespace-disallowed)
    (define-key map "\t" #'mercit-whitespace-disallowed)
    map))

(defun mercit-whitespace-disallowed ()
...
(defun mercit-read-string (prompt &optional initial-input history default-value
...
* an invalid DEFAULT-VALUE is silently ignored."
  ...
  (let* ((minibuffer-completion-table nil)
         (val (read-from-minibuffer
               (mercit-prompt-with-default (concat prompt ": ") default-value)
               initial-input (and no-whitespace mercit-minibuffer-local-ns-map)
...
(defun mercit-read-string-ns (prompt &optional initial-input history
                                    default-value inherit-input-method)
  "Call `mercit-read-string' with non-nil NO-WHITESPACE."
  (mercit-read-string prompt initial-input history default-value
                     inherit-input-method t))

(defmacro mercit-read-char-case (prompt verbose &rest clauses)
  (declare (indent 2)
...
(defun mercit-y-or-n-p (prompt &optional action)
  "Ask user a \"y or n\" or a \"yes or no\" question using PROMPT.
Which kind of question is used depends on whether
ACTION is a member of option `mercit-slow-confirm'."
  (if (or (eq mercit-slow-confirm t)
          (and action (member action mercit-slow-confirm)))
      (yes-or-no-p prompt)
    (y-or-n-p prompt)))

(defvar mercit--no-confirm-alist
  '((safe-with-wip mercit-wip-before-change-mode
                   discard reverse stage-all-changes unstage-all-changes)))

(cl-defun mercit-confirm (action &optional prompt prompt-n noabort
                                (items nil sitems))
  (declare (indent defun))
  (setq prompt-n (format (concat (or prompt-n prompt) "? ") (length items)))
  (setq prompt   (format (concat (or prompt (mercit-confirm-make-prompt action))
                                 "? ")
                         (car items)))
  (or (cond ((and (not (eq action t))
                  (or (eq mercit-no-confirm t)
                      (memq action mercit-no-confirm)
                      (cl-member-if (pcase-lambda (`(,key ,var . ,sub))
                                      (and (memq key mercit-no-confirm)
                                           (memq action sub)
                                           (or (not var)
                                               (and (boundp var)
                                                    (symbol-value var)))))
                                    mercit--no-confirm-alist)))
             (or (not sitems) items))
            ((not sitems)
             (mercit-y-or-n-p prompt action))
            ((length= items 1)
             (and (mercit-y-or-n-p prompt action) items))
            ((length> items 1)
             (and (mercit-y-or-n-p (concat (mapconcat #'identity items "\n")
                                          "\n\n" prompt-n)
                                  action)
                  items)))
      (if noabort nil (user-error "Abort"))))

(defun mercit-confirm-files (action files &optional prompt)
  (when files
    (unless prompt
      (setq prompt (mercit-confirm-make-prompt action)))
    (mercit-confirm action
      (concat prompt " %s")
      (concat prompt " %i files")
      nil files)))

(defun mercit-confirm-make-prompt (action)
  (let ((prompt (symbol-name action)))
    (string-replace "-" " "
                    (concat (upcase (substring prompt 0 1))
                            (substring prompt 1)))))

(defun mercit-read-number-string (prompt &optional default _history)
...
(defmacro mercit-bind-match-strings (varlist string &rest body)
...
(defun mercit-delete-line ()
  "Delete the rest of the current line."
  (delete-region (point) (1+ (line-end-position))))

(defun mercit-delete-match (&optional num)
...
(defun mercit-file-lines (file &optional keep-empty-lines)
...
(defun mercit-set-header-line-format (string)
  "Set the header-line using STRING.
Propertize STRING with the `mercit-header-line'.  If the `face'
...
(defun mercit--format-spec (format specification)
...
(defun mercit-kill-this-buffer ()
  "Kill the current buffer."
  (interactive)
  (kill-buffer (current-buffer)))

(defun mercit--buffer-string (&optional min max trim)
...
(when (< emacs-major-version 26)
  ...
  (defun mercit-completion-pcm--all-completions (prefix pattern table pred)
...
(defun mercit-custom-initialize-reset (symbol exp)
...
(defun mercit-hook-custom-get (symbol)
...
;;;###autoload
(defun Info-follow-nearest-node--mercit-gitman (fn &optional fork)
  (let ((node (Info-get-token
               (point) "\\*note[ \n\t]+"
               "\\*note[ \n\t]+\\([^:]*\\):\\(:\\|[ \n\t]*(\\)?")))
    (if (and node (string-match "^(gitman)\\(.+\\)" node))
        (pcase mercit-view-git-manual-method
          ...
          (_
           (user-error "Invalid value for `mercit-view-git-manual-method'")))
      (funcall fn fork))))

;;;###autoload
(advice-add 'Info-follow-nearest-node :around
            #'Info-follow-nearest-node--mercit-gitman)

;; When making changes here, then also adjust the copy in docs/Makefile.
;;;###autoload
(advice-add 'org-man-export :around #'org-man-export--mercit-gitman)
;;;###autoload
(defun org-man-export--mercit-gitman (fn link description format)
...
(defun mercit--straight-chase-links (filename)
...
(if (fboundp 'with-connection-local-variables)
    (defalias 'mercit--with-connection-local-variables
      #'with-connection-local-variables)
  (defmacro mercit--with-connection-local-variables (&rest body)
...
(defun mercit-message (format-string &rest args)
  "Display a message at the bottom of the screen, or not.
Like `message', except that if the users configured option
`mercit-no-message' to prevent the message corresponding to
FORMAT-STRING to be displayed, then don't."
  (unless (--first (string-prefix-p it format-string) mercit-no-message)
    (apply #'message format-string args)))

(defun mercit-msg (format-string &rest args)
...
(defmacro mercit--with-temp-position (buf pos &rest body)
  (declare (indent 2))
